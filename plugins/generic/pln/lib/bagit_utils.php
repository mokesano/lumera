...
{
    $len = strlen($suffix);
    // PHP 8: substr_compare is still valid, but check for empty suffix
    if ($len == 0) {
        return true;
    }
...
function rls($dir) 
{
    ...
    while (count($queue) > 0) {
        ...
        // [PHP 8 FIX] Handle cases where scandir fails
        $scanned = scandir($current);
        if ($scanned === false) continue;
...
function rrmdir($dir)
{
    if (is_dir($dir)) {
        $objects = scandir($dir);
        if ($objects === false) return; // [PHP 8 FIX] Safety check
...
function tmpdir($prefix='bag')
{
    ...
    unlink($dir);
    // [PHP 8 FIX] Explicit mode as integer (0700) is standard, but PHP 8 supports implicit
    // Replaced mkdir call if needed, but original is fine.
    return $dir;
}
...
function saveUrl($url, $filename)
{
    ...
    // [SECURITY FIX] Add timeout to prevent hanging
 ...
 * @param string|null $default   The default value to return. Defaults to null.
 *
 * @return string|null The name of the first existing file.
 */
...
function BagIt_readBagItFile($filename)
{
    ...
    if (file_exists($filename)) {
        ...
        $versions = ['major' => 0, 'minor' => 96];
        $fileEncoding = 'UTF-8';
    }

    return [$versions, $fileEncoding, $errors];
}
 ...
 *
 * @return array|null A two-item array containing the version string as
...
function BagIt_parseVersionString($bagitFileData)
{
    ...
    if ($success) {
        $major = (int)$matches[1];
        $minor = (int)$matches[2];
        // Note: PHP 8 (int) casting handles null implicitly, but logic check kept
...
function BagIt_validateExists($filename, &$errors)
{
    if (! file_exists($filename)) {
        ...
        array_push(
            $errors,
            [$basename, "$basename does not exist."]
        );
        return false;
    }
    return true;
 ...
 *
 * @param array  $map  The associative array containing the current value.
 * @param string $key  The key storing the current value.
 * @param mixed  $val  The new value to add to the array under the given key.
 *
