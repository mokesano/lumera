...
<div id="articles">
	<div><a class="link-button" href="{plugin_url path="issues"}">{translate key="plugins.importexport.crossref.manageIssues"}</a></div>
	<br />
	<ul class="menu">
				...
				{if $article->getPubId('doi')}
					{if ($filter && $filter == $status) || !$filter}
						...
						<tr valign="top">
							...
							<td><a href="{url page="article" op="view" path=$article->getId()}" class="action truncate-title">{$article->getLocalizedTitle()|nl2br}</a></td>
