...
class NativeExportDom {
    ...
    public function __construct() {
        // Static utility class, usually not instantiated, but provided for consistency.
    }

    /**
     * [SHIM] Backward Compatibility
     */
    public function NativeExportDom() {
     ...
     * @return DOMElement
     */
    public static function generateIssueDom(DOMDocument $doc, $journal, $issue): DOMElement {
        $root = XMLCustomWriter::createElement($doc, 'issue');

        self::generatePubId($doc, $root, $issue, $issue);

        XMLCustomWriter::setAttribute($root, 'published', $issue->getPublished() ? 'true' : 'false');

        $identificationCode = (int) $issue->getShowVolume() .
        ...
        XMLCustomWriter::setAttribute($root, 'identification', (string) $idType, false);
        ...
        XMLCustomWriter::createChildWithText($doc, $root, 'number', (string) $issue->getNumber(), false);
        XMLCustomWriter::createChildWithText($doc, $root, 'year', (string) $issue->getYear(), false);
            ...
            foreach (array_keys($issue->getShowCoverPage(null)) as $locale) {
                if ($issue->getShowCoverPage($locale)) {
                    ...
                    if ($coverFile != '') {
                        ...
                        $fileContent = $publicFileManager->readFile($coverPagePath);
                        // base64_encode expects string, ensure it's not false
                        $encodedContent = $fileContent !== false ? base64_encode($fileContent) : '';
        ...
        if (XMLCustomWriter::createChildWithText($doc, $root, 'access_date', self::formatDate($issue->getOpenAccessDate()), false) == null) {
            // This may be an open access issue. Check and flag as necessary.
     ...
     * @return DOMElement
     */
    public static function generateSectionDom(DOMDocument $doc, $journal, $issue, $section): DOMElement {
        ...
        if (is_array($section->getAbbrev(null))) {
            foreach ($section->getAbbrev(null) as $locale => $abbrev) {
        ...
        if (is_array($section->getIdentifyType(null))) {
            foreach ($section->getIdentifyType(null) as $locale => $identifyType) {
        ...
        if (is_array($section->getPolicy(null))) {
            foreach ($section->getPolicy(null) as $locale => $policy) {
     ...
     * @return DOMElement
     */
    public static function generateArticleDom(DOMDocument $doc, $journal, $issue, $section, $article): DOMElement {
        ...
        self::generatePubId($doc, $root, $article, $issue);
        ...
        foreach ($article->getAuthors() as $author) {
            $authorNode = self::generateAuthorDom($doc, $journal, $issue, $article, $author);
        ...
        XMLCustomWriter::createChildWithText($doc, $root, 'pages', (string) $article->getPages(), false);
        ...
        foreach ($article->getGalleys() as $galley) {
            $galleyNode = self::generateGalleyDom($doc, $journal, $issue, $article, $galley);
            if ($galleyNode !== null) XMLCustomWriter::appendChild($root, $galleyNode);
        }

        /* --- Supplementary Files --- */
        foreach ($article->getSuppFiles() as $suppFile) {
            $suppNode = self::generateSuppFileDom($doc, $journal, $issue, $article, $suppFile);
     ...
     * @return DOMElement
     */
    public static function generateAuthorDom(DOMDocument $doc, $journal, $issue, $article, $author): DOMElement {
        ...
        if (is_array($affiliations)) {
            foreach ($affiliations as $locale => $affiliation) {
        ...
        if (is_array($author->getCompetingInterests(null))) {
            foreach ($author->getCompetingInterests(null) as $locale => $competingInterests) {
     ...
     */
    public static function generateGalleyDom(DOMDocument $doc, $journal, $issue, $article, $galley): ?DOMElement {
        ...
        self::generatePubId($doc, $root, $galley, $issue);
        ...
        if ($galley->getRemoteURL()) {
            ...
            $embedNode = XMLCustomWriter::createChildWithText($doc, $fileNode, 'embed', $encodedContent);
            $articleFile = $articleFileDao->getArticleFile($galley->getFileId());
            if (!$articleFile) return null; // Stupidity check
            ...
            if ($isHtml) {
                ...
                if ($styleFile) {
                    ...
                    $styleContent = $articleFileManager->readFile($styleFile->getFileId());
                    $encodedStyle = $styleContent !== false ? base64_encode($styleContent) : '';

                    $embedNode = XMLCustomWriter::createChildWithText($doc, $styleNode, 'embed', $encodedStyle);
                ...
                foreach ($galley->getImageFiles() as $imageFile) {
                    ...
                    $imageContent = $articleFileManager->readFile($imageFile->getFileId());
                    $encodedImage = $imageContent !== false ? base64_encode($imageContent) : '';

                    $embedNode = XMLCustomWriter::createChildWithText($doc, $imageNode, 'embed', $encodedImage);
    ...
    /**
     * Generate Supplemental File DOM
     * @param DOMDocument $doc
     * @param object $journal
     * @param object $issue
     * @param object $article
     * @param object $suppFile
     * @return DOMElement
     */
    public static function generateSuppFileDom(DOMDocument $doc, $journal, $issue, $article, $suppFile): DOMElement {
        $root = XMLCustomWriter::createElement($doc, 'supplemental_file');

        self::generatePubId($doc, $root, $suppFile, $issue);
        ...
        XMLCustomWriter::createChildWithText($doc, $root, 'date_created', self::formatDate($suppFile->getDateCreated()), false);
        ...
        if ($suppFile->getRemoteURL()) {
            ...
            $fileContent = $articleFileManager->readFile($suppFile->getFileId());
            $encodedContent = $fileContent !== false ? base64_encode($fileContent) : '';
     ...
     * @param object $issue
     */
    public static function generatePubId(DOMDocument $doc, DOMNode $node, $pubObject, $issue): void {
