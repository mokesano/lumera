	...
	<div class="u-full-height" data-native-ad-placement="false">
		<article class="u-full-height c-card c-card--flush" itemscope="" itemtype="http://schema.org/ScholarlyArticle">
			<div class="c-card__layout u-full-height">
				...
				<div class="c-card__image">
					<picture>
						...
						<img src="{$coverPagePath|escape}{$article->getLocalizedFileName()|escape}" alt="" itemprop="image">
					</picture>
				</div>
				{/if}
				...
				<div class="c-card__body u-display-flex u-flex-direction-column">
					...
					{if $article->getLocalizedAbstract()}
					<div class="c-card__summary u-mb-16 u-hide-sm-max" itemprop="description"><p>{$article->getLocalizedAbstract()|strip_tags|nl2br|truncate:127:"..."}</p></div>
					{/if}
				</div>
			</div>
			<div class="c-card__section c-meta">
				...
				{/if}
				{if $section.title}<span class="c-meta__item" data-test="article.type"><span class="c-meta__type">{$section.title|escape|capitalize|replace:'Research':''}</span></span>{/if}
				{if $currentJournal->getSetting('publishingMode') == $smarty.const.PUBLISHING_MODE_OPEN || $article->getAccessStatus() == $smarty.const.ARTICLE_ACCESS_OPEN}<span class="c-meta__item" itemprop="openAccess" data-test="open-access"><span class="u-color-open-access">{translate key="reader.openAccess"}</span></span>{/if}
				<time class="c-meta__item" datetime="{$article->getDatePublished()|date_format:"$dateFormatShort"}" itemprop="datePublished">{$article->getDatePublished()|date_format:"%d %b %Y"}</time>
				<span class="c-meta__item" data-test="article.pages">{if $article->getPages()}P {$article->getPages()|escape}{else}e{$article->getId()}{/if}</span>
			</div>
		</article>
	</div>
</li>
