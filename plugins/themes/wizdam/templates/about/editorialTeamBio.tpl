            	...
            	<div class="sc-uosj0-0 AcWHr">
            	    <a data-aa-name="Journal Cover" data-aa-region="header" href="{url journal=$currentJournal->getPath()}" class="sc-uosj0-1 fAiWUX header-journal-cover">
            	        ...
            	        <picture sangiaHome>
            	            ...
            	            <img src="{$publicFilesDir}/{$displayHomepageImage.uploadName|escape:"url"}" width="75" height="auto" alt="Journal cover for {$currentJournal->getLocalizedTitle()|strip_tags|escape}" loading="lazy" />
            	        </picture>
            	        ...
            	        <picture sangia>
            	            ...
            	            <img src="{$publicFilesDir}/{$displayJournalThumbnail.uploadName|escape:"url"}" width="75" height="auto" alt="Journal cover for {$currentJournal->getLocalizedTitle()|strip_tags|escape}" loading="lazy" />
            	        </picture>
            	        {else}
            	        <picture>
            	            <source srcset="//assets.sangia.org/img/img-default.jpg?as=webp" type="image/webp" loading="lazy">
            	            <img src="//assets.sangia.org/img/img-default.jpg" width="75" height="auto" alt="Journal cover for {$currentJournal->getLocalizedTitle()|strip_tags|escape}" loading="lazy" />
            	        </picture>
            	        {/if}
            	    </a>
...
<section class="live-area-wrapper">
    <div class="profile row">
        ...
        <section class="column medium-10 cms-person">
            <h2 class="editor-name" itemprop="editors" itemtype="http://schema.org/Person">
                {if $user->getSalutation()}<span class="italic" itemprop="salutation">{$user->getSalutation()}</span>{/if}
                {if $user->getFirstName() !== $user->getLastName()}<span itemprop="given-name">{$user->getFirstName()}</span>{/if}
                {if $user->getMiddleName()}<span itemprop="middle-name">{$user->getMiddleName()}</span>{/if}
                <span itemprop="surname">{$user->getLastName()}</span>
            </h2>
        </section>
    </div>
    <div class="profile row">
		<section class="column medium-2">
			...
			<div class="avatar bJniPh islvFm" size="150">
			    <span style="box-sizing: border-box; display: inline-block; overflow: hidden; width: 100%; height: 100%; background: rgba(0, 0, 0, 0) none repeat scroll 0% 0%; opacity: 1; border: 0px none; margin: 0px; padding: 0px; position: relative; max-width: -moz-available;">
				    <span style="box-sizing: border-box; display: block; width: 100%; height: 100%; background: rgba(0, 0, 0, 0) none repeat scroll 0% 0%; opacity: 1; border: 0px none; margin: 0px; padding: 0px; max-width: 150px;">
				        <img style="display: block; max-width: 100%; width: 100%; height: initial; background: rgba(0, 0, 0, 0) none repeat scroll 0% 0%; opacity: 1; border: 0px none; margin: 0px; padding: 0px;" alt="" aria-hidden="true" src="//assets.sangia.org/img/128x150-image.png">
				    </span>
				    <picture>
				    {if $profileImage}
					<img loading="lazy" id="editorid={$editors}" height="auto" width="150" title="{$user->getData('salutation')} {$user->getFullName()|escape}" class="avatar editor sangia-author" src="{$sitePublicFilesDir}/{$profileImage.uploadName}" style="position: absolute; inset: 0px; box-sizing: border-box; padding: 0px; border: medium none; margin: auto; display: block; width: 0px; height: 0px; min-width: 100%; max-width: 100%; min-height: 100%; max-height: 100%; object-fit: cover;" />
					{else}
					<img loading="lazy" id="ID-{$editors}" class="avatar editor sangia-author" title="{$user->getData('salutation')} {$user->getFullName()|escape}" {if $user->getGender() == "M"}src="//assets.sangia.org/static/images/contactPersonM.png"{elseif $user->getGender() == "F"}src="//assets.sangia.org/static/images/contactPersonF.png"{elseif $user->getGender() == "O"}src="//scholar.google.co.id/citations/images/avatar_scholar_128.png"{elseif $user->getGender() == ""}src="//scholar.google.co.id/citations/images/avatar_scholar_128.png"{/if} width="150" height="auto" style="position: absolute; inset: 0px; box-sizing: border-box; padding: 0px; border: medium none; margin: auto; display: block; width: 0px; height: 0px; min-width: 100%; max-width: 100%; min-height: 100%; max-height: 100%; object-fit: cover;" />
				    {/if}
				    </picture>
                </span>
			</div>		
		...
		<section class="column medium-7 cms-person">
			<div class="overview description">
                <p class="paragraph" itemprop="creator" itemtype="http://schema.org/Person"><span class="editors" itemprop="name">Editorial Board Members</span>
                </p>			    
                ...
                <div class="sc-1mur6on-9 bLswwL GooSSiP">
            		...
            		<p class="sc-1q3g1nv-0 sc-1mur6on-13 eTETae fziRAl" itemprop="affiliation" itemtype="http://schema.org/Affiliation">{$user->getLocalizedGossip()}</p>
                </div>
                {/if}
				...
				<div class="editor-person eTETae">
					<p class="sc-1q3g1nv-0 sc-1mur6on-13 fziRAl">
						{if $user->getData('orcid')}<img src="{$baseUrl}/public/site/images/orcid_16x16.svg" style="height:16px" alt="orcid" /> <span class="orcid">{translate key="user.orcid"}</span> <a title="Go to Orcid profile of {$user->getData('salutation')} {$user->getFullName()|escape}" href="{$user->getData('orcid')|escape}" target="_blank">{$user->getData('orcid')|escape}</a>{/if}
					</p>
					<p class="sc-1q3g1nv-0 sc-1mur6on-13 fziRAl">
						{if $user->getData('phone')}<span class="sinta">Sinta: </span><a title="Go to Sinta profile of {$user->getData('salutation')} {$user->getFullName()|escape}" href="//sinta.kemdikbud.go.id/authors/profile/{$user->getData('phone')|escape}" target="_blank" class="scopusid">Science And Technology Index</a><span class="articleCount"> Academic Profile by RISTEK/BRIN Indonesia</span>{/if}
					</p>
					<p class="sc-1q3g1nv-0 sc-1mur6on-13 fziRAl">
						{if $user->getData('fax')}<span class="scopus">Scopus</span> Author ID <a title="Go to Scopus profile of {$user->getData('salutation')} {$user->getFullName()|escape}" href="https://www.scopus.com/authid/detail.uri?authorId={$user->getData('fax')|escape}" target="_blank" class="scopusid">{$user->getData('fax')|escape}</a>{/if}
					</p>
				</div>
				
				{if $user->getLocalizedGossip()}
				<div class="sc-1mur6on-15 fTcNcY">
				    <a href="mailto:{$user->getEmail()|escape}" class="sc-kvjqii-0 fYuYRT sc-1mur6on-16 bneccj"><span class="sc-kvjqii-1 hjlxNa"><svg aria-hidden="true" focusable="false" viewBox="0 0 15 15" height="2em" width="2em"><path d="M.156 13.469L6.094 7.53.156 1.594 1.25.5l7.031 7.031-7.031 7.032z" fill="currentColor" fill-rule="nonzero"></path></svg></span>
				        <span style="font-size: initial;">Contact {$user->getData('salutation')} {$user->getFullName()|escape}</span>
				    </a>
				</div>
				{/if}
			</div>	    
		...
		<aside class="column medium-3">
			<section class="box">
				<section class="description" title="Phone number and email addresses editor/reviewer in {$currentJournal->getLocalizedTitle()|strip_tags|escape} are not displayed. See more about Privacy Statements.">
				    <p class="headline-524909129 __info">Available Contact{if $user->getData('phone')}s{/if}</p>
...
<div class="live-area-wrapper">
    <div class="profile row">
        <aside class="column medium-2 null">
    		...
    		{/if}
    		<section class="ads{if $user->getLocalizedBiography() || $user->getLocalizedSignature() || $user->getData('mailingAddress')}{else} u-hide{/if}">
    	...
    	<div class="column medium-10">
    		<div class="person-detail">{$user->getLocalizedBiography()|nl2br|strip_unsafe_html} {if $user->getUrl()}<a title="Go to Editor's personal profile of {$user->getFullName()|escape}" href="{$user->getUrl()|escape}" target="_blank">Editor's Personal Website</a>{/if}
    		    <p>{$user->getLocalizedSignature()|nl2br|strip_unsafe_html}</p>
    		    <div class="Address u-mt-16">
    		        <p>{$user->getData('mailingAddress')|nl2br|strip_unsafe_html}</p>
    		    </div>
    		</div>
    	</div>
