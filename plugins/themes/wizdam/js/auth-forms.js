 ...
 * 
 * @fileoverview Modern authentication forms with floating labels, password validation,
 * email verification, and professional checkbox designs for OJS registration system.
 * @version 1.2.2
 * @author OJS Theme Developer
 * @requires none
 */
...
(function() {
    ...
    /**
     * Initialize password strength indicator for both login and register
     * @function
     * @returns {void}
     */
    function initializePasswordStrengthIndicator() {
        const registerPasswordInput = document.getElementById('password');
        const registerStrengthIndicator = document.getElementById('passwordStrengthIndicator');
        
        const loginPasswordInput = document.getElementById('loginPassword');
        const loginStrengthIndicator = document.getElementById('loginPasswordStrengthIndicator');
        
        if (registerPasswordInput && registerStrengthIndicator) {
            const currentPassword = registerPasswordInput.value || '';
            updatePasswordRequirements(currentPassword);
            updateInputState(registerPasswordInput);
            updateFieldMessages(registerPasswordInput);
        ...
        if (loginPasswordInput) {
            ...
            updatePasswordRequirements(currentPassword);
            updateInputState(loginPasswordInput);
            updateFieldMessages(loginPasswordInput);
            ...
            loginPasswordInput.addEventListener('focus', function() {
                if (loginStrengthIndicator && !loginStrengthIndicator.parentNode) {
                    loginPasswordInput.parentElement.parentElement.appendChild(loginStrengthIndicator);
                }
            });
            
            loginPasswordInput.addEventListener('blur', function() {
                if (loginStrengthIndicator && !this.value.trim() && loginStrengthIndicator.parentNode) {
                    loginStrengthIndicator.parentNode.removeChild(loginStrengthIndicator);
                }
            });
        }
    }
    ...
    /**
     * Insert message element in correct position (before form-help-text if exists)
     * @function
    ...
    /**
     * Update field messages based on field state
     * @function
    ...
    function updateFieldMessages(field) {
        const fieldContainer = field.parentElement;
        const currentErrorMessage = fieldContainer.querySelector('.error-message');
        const currentSuccessMessage = fieldContainer.querySelector('.success-message');
        
        // Remove existing messages from DOM
        ...
        if (field.type === 'password' && (field.name === 'password' || field.id === 'password' || field.id === 'loginPassword')) {
    ...
    function setupFloatingLabels() {
        ...
        inputs.forEach(function(input) {
            ...
            input.addEventListener('input', function() {
                ...
                if (this.id === 'password' || this.id === 'loginPassword') {
                    updatePasswordRequirements(this.value);
                }
    ...
    function updatePasswordStrengthIndicator(password, metCount, requirements) {
        ...
        const loginStrengthLabel = document.getElementById('loginStrengthLabel');
        const registerStrengthLabel = document.getElementById('strengthLabel');
        
        const strengthIndicator = loginStrengthIndicator || registerStrengthIndicator;
        const strengthLabel = loginStrengthLabel || registerStrengthLabel;
    ...
    function initializeRememberMeState(checkbox) {
        try {
            ...
            console.warn('Could not access localStorage for remember me state');
        }
    }
    ...
    function handleEnhancedRememberMeChange(checkbox) {
        try {
            ...
            console.warn('Could not save remember me state');
        }
    ...
    window.ojsEnhancedUtils = {
        ...
        clearRememberMeState: function() {
            try {
                ...
                console.warn('Could not clear remember me state');
            }
        },
