...
{if !empty($manager)} <!-- settings for view EDITOR-IN-CHIEF-->
	<div id="editors">
	{if count($manager) == 1}
...
{else}
    <div id="editors">
        <p><i>{$currentJournal->getLocalizedTitle()|strip_tags|escape} </i>is in the process of recruiting an {translate key="about.editorialTeam"}</p>
    </div>
{/if}

{if !empty($sectionEditors)}
...
{if !empty($reviewers)} <!-- settings for view REVIEWER -->
	<div id="sectionEditors">
