...
{/strip}
<div id="aboutPeople">
<h2 class="sub_title">{translate key="about.people"}</h2>
<ul>
	...
	{/if}
	<li id="editorialTeamLink"><a href="{url op="editorial-team"}">{translate key="about.editorialTeam"}</a></li>
	{if $peopleGroups}
...
<h2 class="sub_title">{translate key="about.policies"}</h2>
<ul>
	{if $currentJournal->getLocalizedSetting('focusScopeDesc') != ''}<li><a href="{url op="editorial-policies" anchor="focusAndScope"}">{translate key="about.focusAndScope"}</a></li>{/if}
	<li><a href="{url op="editorial-policies" anchor="sectionPolicies"}">{translate key="about.sectionPolicies"}</a></li>
	{if $currentJournal->getLocalizedSetting('reviewPolicy') != ''}<li><a href="{url op="editorial-policies" anchor="peerReviewProcess"}">{translate key="about.peerReviewProcess"}</a></li>{/if}
	{if $currentJournal->getLocalizedSetting('pubFreqPolicy') != ''}<li><a href="{url op="editorial-policies" anchor="publicationFrequency"}">{translate key="about.publicationFrequency"}</a></li>{/if}
	{if $currentJournal->getSetting('publishingMode') == $smarty.const.PUBLISHING_MODE_OPEN && $currentJournal->getLocalizedSetting('openAccessPolicy') != ''}<li><a href="{url op="editorial-policies" anchor="openAccessPolicy"}">{translate key="about.openAccessPolicy"}</a></li>{/if}
	{if $journalSettings.enableLockss && $currentJournal->getLocalizedSetting('lockssLicense') != ''}<li><a href="{url op="editorial-policies" anchor="archiving"}">{translate key="about.archiving"}</a></li>{/if}
	...
	{if $currentJournal->getSetting('publishingMode') == $smarty.const.PUBLISHING_MODE_SUBSCRIPTION}
		...
		{if !empty($journalSettings.enableAuthorSelfArchive)}<li><a href="{url op="editorial-policies" anchor="authorSelfArchivePolicy"}">{translate key="about.authorSelfArchive"}</a></li>{/if}
		{if !empty($journalSettings.enableDelayedOpenAccess)}<li><a href="{url op="editorial-policies" anchor="delayedOpenAccessPolicy"}">{translate key="about.delayedOpenAccess"}</a></li>{/if}
		{if $paymentConfigured && $journalSettings.journalPaymentsEnabled && $journalSettings.acceptSubscriptionPayments && $journalSettings.purchaseIssueFeeEnabled && $journalSettings.purchaseIssueFee > 0}<li><a href="{url op="editorial-policies" anchor="purchaseIssue"}">{translate key="about.purchaseIssue"}</a></li>{/if}
		{if $paymentConfigured && $journalSettings.journalPaymentsEnabled && $journalSettings.acceptSubscriptionPayments && $journalSettings.purchaseArticleFeeEnabled && $journalSettings.purchaseArticleFee > 0}<li><a href="{url op="editorial-policies" anchor="purchaseArticle"}">{translate key="about.purchaseArticle"}</a></li>{/if}
	{/if}{* $currentJournal->getSetting('publishingMode') == $smarty.const.PUBLISHING_MODE_SUBSCRIPTION *}
	{foreach key=key from=$customAboutItems item=customAboutItem}
		{if $customAboutItem.title!=''}<li><a href="{url op="editorial-policies" anchor=custom-$key}">{$customAboutItem.title|escape}</a></li>{/if}
	{/foreach}
	{call_hook name="Templates::About::Index::Policies"}
</ul>
</div>
<div id="aboutSubmissions">
<h2 class="sub_title">{translate key="about.submissions"}</h2>
<ul>
...
<ul>
	...
	<li><a href="{url op="insights"}">{translate key="about.aboutThisPublishingSystem"}</a></li>
