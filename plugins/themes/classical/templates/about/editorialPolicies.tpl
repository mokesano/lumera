...
<ul>
	{if $currentJournal->getLocalizedSetting('focusScopeDesc') != ''}<li id="linkFocusScopeDesc"><a href="{url op="editorial-policies" anchor="focusAndScope"}">{translate key="about.focusAndScope"}</a></li>{/if}
	<li id="linkEditorialPolicies"><a href="{url op="editorial-policies" anchor="sectionPolicies"}">{translate key="about.sectionPolicies"}</a></li>
	{if $currentJournal->getLocalizedSetting('reviewPolicy') != ''}<li id="linkReviewPolicy"><a href="{url op="editorial-policies" anchor="peerReviewProcess"}">{translate key="about.peerReviewProcess"}</a></li>{/if}
	{if $currentJournal->getLocalizedSetting('pubFreqPolicy') != ''}<li id="linkPubFreqPolicy"><a href="{url op="editorial-policies" anchor="publicationFrequency"}">{translate key="about.publicationFrequency"}</a></li>{/if}
	{if $currentJournal->getSetting('publishingMode') == $smarty.const.PUBLISHING_MODE_OPEN && $currentJournal->getLocalizedSetting('openAccessPolicy') != ''}<li id="linkOpenAccessPolicy"><a href="{url op="editorial-policies" anchor="openAccessPolicy"}">{translate key="about.openAccessPolicy"}</a></li>{/if}
	{if $currentJournal->getSetting('publishingMode') == $smarty.const.PUBLISHING_MODE_SUBSCRIPTION}
		{if $currentJournal->getSetting('enableAuthorSelfArchive')}<li id="enabledAuthorSelfArchive"><a href="{url op="editorial-policies" anchor="authorSelfArchivePolicy"}">{translate key="about.authorSelfArchive"}</a></li>{/if}
		{if $currentJournal->getSetting('enableDelayedOpenAccess')}<li id="enabledDelayedOpenAccess"><a href="{url op="editorial-policies" anchor="delayedOpenAccessPolicy"}">{translate key="about.delayedOpenAccess"}</a></li>{/if}
		{if $paymentConfigured && $currentJournal->getSetting('journalPaymentsEnabled') && $currentJournal->getSetting('acceptSubscriptionPayments') && $currentJournal->getSetting('purchaseIssueFeeEnabled') && $currentJournal->getSetting('purchaseIssueFee') > 0}<li id="purchaseIssue"><a href="{url op="editorial-policies" anchor="purchaseIssue"}">{translate key="about.purchaseIssue"}</a></li>{/if}
	...
	{/if}
	{if $currentJournal->getSetting('enableLockss') && $currentJournal->getLocalizedSetting('lockssLicense') != ''}<li id="linkLockssLicense"><a href="{url op="editorial-policies" anchor="archiving"}">{translate key="about.archiving"}</a></li>{/if}
	{foreach key=key from=$currentJournal->getLocalizedSetting('customAboutItems') item=customAboutItem}
		{if !empty($customAboutItem.title)}
			<li><a href="{url op="editorial-policies" anchor=custom-$key}">{$customAboutItem.title|escape}</a></li>
		{/if}
	{/foreach}
</ul>
...
<div id="focusAndScope"><h3 class="sub_title">{translate key="about.focusAndScope"}</h3>
...
<div id="sectionPolicies">
	<h3 class="sub_title">{translate key="about.sectionPolicies"}</h3>
{foreach from=$sections item=section}{if !$section->getHideAbout()}

<div class="section--value"> 
	<h4 class="section-titles">{$section->getLocalizedTitle()}</h4>
	{if strlen($section->getLocalizedPolicy()) > 0}
		<p>{$section->getLocalizedPolicy()|nl2br}</p>
	{/if}

<section class="section-editors">
...
<div id="peerReviewProcess">
	<h3 class="sub_title">{translate key="about.peerReviewProcess"}</h3>
...
<div id="publicationFrequency"><h3 class="sub_title">{translate key="about.publicationFrequency"}</h3>
...
<div id="openAccessPolicy"><h3 class="sub_title">{translate key="about.openAccessPolicy"}</h3>
...
<div id="authorSelfArchivePolicy"><h3 class="sub_title">{translate key="about.authorSelfArchive"}</h3> 
...
<div id="delayedOpenAccessPolicy"><h3 class="sub_title">{translate key="about.delayedOpenAccess"}</h3> 
...
{if $currentJournal->getSetting('publishingMode') == $smarty.const.PUBLISHING_MODE_SUBSCRIPTION &&
	...
	$currentJournal->getSetting('purchaseIssueFee') > 0}
		<div id="purchaseIssue">
			<h3 class="sub_title">{translate key="about.purchaseIssue"}</h3>
...
<div id="archiving"><h3 class="sub_title">{translate key="about.archiving"}</h3>
