...
{/strip}
    ...
    <div id="publication" class="Publication">
        ...
        <div class="publication-cover u-show-from-sm journal-page">
            ...
            {assign var="displayHomepageImage" value=$currentJournal->getLocalizedSetting('homepageImage')}
            {if $issue->getBestIssueId($currentJournal)}
            <a rel="noreferrer noopener" href="{url page="issue" op="view" path=$issue->getBestIssueId($currentJournal)}"><img class="publication-cover-image" src="{$publicFilesDir}/{$displayHomepageImage.uploadName|escape:"url"}" loading="lazy" alt="{$currentJournal->getLocalizedTitle()|strip_tags|escape}"/></a>
            {else}
    ...
    <div id="banner" class="Banner">
        ...
        <div class="wrapper truncated">
            ...
            <a rel="noreferrer noopener" href="{url page="search" op="authors" path="view" firstName=$authorFirstName middleName=$authorMiddleName lastName=$authorLastName affiliation=$authorAffiliation country=$authorCountry}" class="author size-m workspace-trigger" target="_blank"><span class="content"><span class="text given-name">{$authorFirstName|escape}</span>{if $authorMiddleName}<span class="text middle-name">{$authorMiddleName|escape}</span>{/if}<span class="text surname">{$authorLastName|escape}</span>{if $authorCount-1}<span class="author-ref" id="baff0010"><sup>{$count|escape}</sup></span>{else}{/if}{if $author->getData('primaryContact')|escape}<svg title="Corresponding author" focusable="false" viewBox="0 0 106 128" class="icon icon-person" height="11" width="12"><path d="m11.07 1.2e2l0.84-9.29c1.97-18.79 23.34-22.93 41.09-22.93 17.74 0 39.11 4.13 41.08 22.84l0.84 9.38h10.04l-0.93-10.34c-2.15-20.43-20.14-31.66-51.03-31.66s-48.89 11.22-51.05 31.73l-0.91 10.27h10.03m41.93-102.29c-9.72 0-18.24 8.69-18.24 18.59 0 13.67 7.84 23.98 18.24 23.98s18.24-10.31 18.24-23.98c0-9.9-8.52-18.59-18.24-18.59zm0 52.29c-15.96 0-28-14.48-28-33.67 0-15.36 12.82-28.33 28-28.33s28 12.97 28 28.33c0 19.19-12.04 33.67-28 33.67"></path></svg>{if $author->getData('email')}<svg title="{$fullname|escape} corresponding mail: {$author->getData('email')|escape}" class="icon icon-envelope" width="12" height="12" viewBox="0.741 0 14 8"><path d="M13.741 0L7.24 5.121.74 0zM.742 1.714L.74 10h6.502l-.001-3.165zm6.501 5.121L7.242 10h6.499V1.714z" alt="mail" /></svg>{/if}{else}{/if}</span>{if $i==$authorCount-2}, {elseif $i<$authorCount-1}, {/if}</a>{/foreach}
                </div>
            </div>
        </div>
        {/if}
                ...
                <div id="author-group" class="author-group">
                    ...
                    <dl class="affiliation">
                        ...
                        {if $authorAffiliation||$count}
                            ...
                            <dt><sup></sup></dt>
                            {/if}
                            <dd>{if $authorAffiliation|escape}{$authorAffiliation|escape}{else}[<i>Author affiliation not available</i>]{/if}{if $author->getCountry()}, {$author->getCountryLocalized()|escape}{/if}
                            </dd>
                        {/if}    
                    </dl>
                    {/foreach}
    ...
    <div class="DoiLink u-font-sans-sang" id="doi-link">
        ...
        {/if}
        {if $pubId}
            {if $pubIdPlugin->getResolvingURL($currentJournal->getId(), $pubId)|escape}<a class="doi" target="_blank" rel="noreferrer noopener" aria-label="Persistent link using digital object identifier" title="Persistent link using digital object identifier" href="{$pubIdPlugin->getResolvingURL($currentJournal->getId(), $pubId)|escape}">{$pubIdPlugin->getResolvingURL($currentJournal->getId(), $pubId)|escape}</a>{else}<a rel="noreferrer noopener" href="javascript:void(0)">DOI not available</a>{/if}
        {else}
    ...
    {if $coverPagePath}
    <div id="abstracts" class="abstract graphical u-font-serif">
...
<div id="body" class="Body u-font-serif">
    ...
    <div class="SubscribeArea">
        <div class="LiveAreaSection-193358632">
            <section class="BuyBoxSection-683559780" data-gtm-vis-recent-on-screen-50443292_563="3930" data-gtm-vis-first-on-screen-50443292_563="3930" data-gtm-vis-total-visible-time-50443292_563="10000" data-gtm-vis-has-fired-50443292_563="1"><div class="subscribe-buybox"><div class="box-inner"><p class="title-buybox">Subscribe to Journal</p><p class="access-buybox">Get full journal access for 1 year</p><div><p class="price-buybox" id="subscription-price">185,98&nbsp;€</p><p class="issue-buybox">only 3,65 € per issue</p></div><div class="button-container"><a href="/nature/subscribe" class="Button-1078489254" data-track="click" data-track-action="subscribe" data-track-label="link" data-track-category="article body"><span class="ButtonLabel-3296148077">Subscribe</span></a></div><p class="vat-buybox">All prices are NET prices.<br> VAT will be added later in the checkout.<br>Tax calculation will be finalised during checkout.</p></div></div><div class="readcube-buybox"><div class="box-inner"><p class="title-readcube">Buy article</p><p class="access-readcube">Get time limited or full article access on ReadCube.</p><p id="readcube-price" class="price-buybox">$32.00</p><div class="button-container"><a href="//www.nature.com/articles/s41586-022-05572-6.epdf?no_publisher_access=1&amp;r3_referer=nature" class="readcube-button Button-2808614501" data-track="click" data-track-action="buy/rent now" data-track-label="link" data-track-category="article body"><span class="readcube-label ButtonLabel-1566022830">Buy</span></a></div><p class="vat-buybox">All prices are NET prices.</p></div></div>
            </section>
        </div>
        <div class="PageDivider"></div>
    </div>
    ...
    <div class="SubscribeArea">
        <h2 class="section-title u-h3 u-margin-l-top u-margin-xs-bottom">Access options</h2>
        <div class="LiveAreaSection-193358632">
            <section class="BuyBoxSection-683559780" data-gtm-vis-recent-on-screen-50443292_563="3930" data-gtm-vis-first-on-screen-50443292_563="3930" data-gtm-vis-total-visible-time-50443292_563="10000" data-gtm-vis-has-fired-50443292_563="1"><div class="subscribe-buybox"><div class="box-inner"><p class="title-buybox">Subscribe to Journal</p><p class="access-buybox">Get full journal access for 1 year</p><div><p class="price-buybox" id="subscription-price">185,98&nbsp;€</p><p class="issue-buybox">only 3,65 € per issue</p></div><div class="button-container"><a href="#" class="Button-1078489254" data-track="click" data-track-action="subscribe" data-track-label="link" data-track-category="article body"><span class="ButtonLabel-3296148077">Subscribe</span></a></div><p class="vat-buybox">All prices are NET prices.<br> VAT will be added later in the checkout.<br>Tax calculation will be finalised during checkout.</p></div></div><div class="readcube-buybox"><div class="box-inner"><p class="title-readcube">Buy article</p><p class="access-readcube">Get time limited or full article access on ReadCube.</p><p id="readcube-price" class="price-buybox">$32.00</p><div class="button-container"><a href="//www.nature.com/articles/s41586-022-05572-6.epdf?no_publisher_access=1&amp;r3_referer=nature" class="readcube-button Button-2808614501" data-track="click" data-track-action="buy/rent now" data-track-label="link" data-track-category="article body"><span class="readcube-label ButtonLabel-1566022830">Buy</span></a></div><p class="vat-buybox">All prices are NET prices.</p></div></div>
            </section>
        </div>
        <div class="PageDivider u-mt-16"></div>
    </div>
        ...
        <section id="s0145">
            <h2 id="st0200" class="u-h3 u-margin-l-top u-margin-xs-bottom">Data availability</h2>
        ...
        <section id="s0135" class="u-hide"><h2 id="st0190" class="u-h3 u-margin-l-top u-margin-xs-bottom">Ethical approval acknowledgements</h2><p id="p0310">No ethical approval required for this article.</p></section>

        <section id="coi0005" class="u-hide"><h2 id="st0205" class="u-h3 u-margin-l-top u-margin-xs-bottom">Competing interest</h2><p id="p0325">No conflict of interest has been declared by the authors.</p></section>
    ...
    <section id="References" class="bibliography u-font-serif text-m">
        <header>
            <h2 class="section-title u-h3 u-margin-l-top u-margin-xs-bottom">
                ...
                <span class="count">({$citationFactory->getCount()})</span>
            </h2>
        </header>
        <section class="ref-bibliography">
            {iterate from=citationFactory item=citation}
            <p id="ref-sangia007" class="reference">{$citation->getRawCitation()}</p>
            {/iterate}
        </section>
...
{foreach from=$authors item=author name=authors}
    ...
    <div class="article-biography author-group" id="vt00{$i+5}">
        ...
        {/if}
        <p id="spar00{$i+15}" class="author">{$authorSalutation|escape}<strong><span class="content"><span class="text given-name">{$authorFirstName|escape}</span>{if $authorMiddleName}<span class="text middle-name">{$authorMiddleName|escape}</span>{/if}<span class="text surname">{$authorLastName|escape},</span></span></strong>{if $author->getLocalizedBiography()} {$author->getLocalizedBiography()|strip_tags|nl2br}. {/if} {$authorAffiliation|escape}{if $author->getCountry()}, {$author->getCountryLocalized()|escape}{/if}.
