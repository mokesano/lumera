...
{foreach from=$userJournals item=journal}
	...
	{else}<h2>{$journal->getLocalizedTitle()|escape}</h2>{/if}
	{assign var="journalId" value=$journal->getId()}
	{assign var="journalPath" value=$journal->getPath()}
	<table width="100%" class="info">
		<tr class="functions-bar functions-bar-top tableHeading">
			<th class="nbsp">&nbsp;User Pages</th>
			<th>Assign</th>
			<th>Review</th>
			<th>Edit</th>
...
<div id="myAccount">
<h2>{translate key="user.myAccount"}</h2>
<ul>
	{if $hasOtherJournals}
		{if !$showAllJournals}
