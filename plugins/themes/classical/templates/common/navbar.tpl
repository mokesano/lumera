 ...
 *
 *}
<div id="header" role="banner" class="show">
	<div class="cross-nav cross-nav--wide">
    	{if $isUserLoggedIn}
        	{if $hasOtherJournals}
            	<div id="userJournal" class="auth flyout"><a class="register-link flyout-caption" href="{url journal="index" page="user"}">My Journals</a></div>
            {/if}
    	...
    	{else}
    		...
    		{if !$hideRegisterLink}
        	<div id="userRegister" class="auth flyout"><a class="register-link flyout-caption" href="{url page="user" op="register"}">{translate key="navigation.register"}</a>
        	</div>
    		{/if}
    	{/if}{* $isUserLoggedIn *}
        ...
        <div class="lang flyout">
            <button class="flyout-caption cur" aria-expanded="false">
                <span>English</span>
                <img class="flyout-caption-icon" src="//assets.sangia.org/image/classical/flyout-arrow.png" alt="">
            </button>
            <div class="flyout-content">
                <ul>
                  <li><a class="change-language-Indonesia" href="/language/Indonesia" rel="nofollow">Indonesia</a></li>
    ...
    <a id="logo" href="{$baseUrl}"><img src="//assets.sangia.org/img/sangia-black-branded-v3.svg" alt="Logo Sangia Publishing Group" title="Sangia for Research & Development"></a>
    <div class="panel-search">
        <form id="global-search" class="big-search lm-site-search {if $query}contains-new-search-link{/if}{if $subject}contains-new-search-link{/if}" action="{url page="search" op="search"}" method="get" role="search">
            ...
            <div class="search-field">
                ...
                {if $query}
                <a id="global-search-new" {if $siteSearch}href="{url page="index" op="search"}"{else}href="{url page="search" op="search"}"{/if}>New Search</a>
                {/if}
                {if $subject}
                <a id="global-search-new" {if $siteSearch}href="{url page="index" op="search"}"{else}href="{url page="search" op="search"}"{/if}>New Search</a>
                {/if}
            {else}
        		{$queryFilter}
        	{/if}
            <input id="search" class="search-submit" type="submit" value="Submit"><img class="loading js-hidden" src="//assets.sangia.org/image/classical/loading_search.gif" alt="">
            <img class="search-submit-image" src="//assets.sangia.org/image/classical/search-submit.png" alt="Search">
            <div id="search-options" class="flyout">
                <button class="pillow-btn open-search-options" type="button" aria-expanded="false">
                    <img src="//assets.sangia.org/image/classical/settings-active.png" alt="Search Options">
                </button>
                <div class="flyout-content">
                    <ul>
                        <li><a id="advanced-search-link" href="/advanced-search">Advanced Search</a></li>
                        <li><a id="search-help-link" href="/searchhelp">Search Help</a></li>
                    </ul>
                </div>
            </div>
        </form>
    ...
    <div class="panel-menu">
        ...
        <ul id="global-nav" class="clearfix" role="navigation" aria-label="Global">
            <li id="home"><a href="{url journal=$currentJournal->getPath()}" >{translate key="navigation.home"}</a></li>
    		...
    		{/if}
    		<li id="categories"><a href="{url page="search" op="categories"}" >Journal A-Z</a></li>
    		{if $currentJournal->getSetting('publishingMode') == $smarty.const.PUBLISHING_MODE_SUBSCRIPTION || $donationEnabled || $currentJournal->getSetting('membershipFee')}
        		...
        		<li id="articles"><a href="{url page="about" op="subscriptions"}" >Subscribe</a></li>
        		{/if}{* $currentJournal->getSetting('publishingMode') == $smarty.const.PUBLISHING_MODE_SUBSCRIPTION *}
        		{if $currentJournal->getSetting('donationFeeEnabled')}
        		<li id="articles"><a href="{url page="donations"}">Donation</a></li>
        		{/if}
        		{if $currentJournal->getSetting('membershipFeeEnabled')}
        		<li id="articles"><a href="{url page="about" op="memberships"}" >{translate key="about.memberships"}</a></li>
        		{/if}
    		{/if}
