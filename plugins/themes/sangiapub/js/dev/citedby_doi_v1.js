/**
 * @file public/assets/sangiapub/js/dev/citedby_doi_v1.js
 * 
...
(function() {
   ...
   const refreshButtonTemplate = (sourcesInfo) => `
       <div class="tooltip-wrap">
           ...
           <div class="citation-tooltip">${sourcesInfo || 'Refresh citations'}</div>
       </div>
   `;
           ...
           .then(response => {
               if (response.status === 'success') {
                   ...
                   if (citationCount > 0) {
                       ...
                       infoDiv.innerHTML = `<span class="update-info">Last updated: ${formattedDate}</span>`;
                       ...
                       if (refreshButton) {
                           refreshButton.addEventListener('click', function() {
                               if (!citingContainer.querySelector('.citation-refresh-overlay')) {
                                   showLoading(citingContainer);
                   ...
                   hideCitationPanelIfEmpty(0);
                   console.error('Citation fetch error:', response.message || 'Unknown error');
               }
           })
           .catch(error => {
               // Error handling
