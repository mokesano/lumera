...
<div class="c-jump">
    ...
    <span class="c-jump-navigation">{foreach from=$alphaList item=letter}<a class="c-jump-navigation__link u-margin-bottom-xxs-at-md" href="{url path=$group->getId() searchInitial=$letter}">{if $letter == $searchInitial}<strong>{$letter|escape}</strong>{else}{$letter|escape}{/if}</a> {/foreach}<a class="c-jump-navigation__link u-margin-bottom-xxs-at-md" href="{url path=$group->getId()}">{if $searchInitial==''}<strong>{translate key="common.all"}</strong>{else}{translate key="common.all"}{/if}</a></span>
</div>
...
<div id="users">
    <table width="100%" class="listing">
        ...
        <tr class="heading" valign="bottom">
        	<td width="10%">{translate key="user.username"}</td>
        	<td width="70%">{translate key="user.name"}</td>
    ...
    <a class="u-js-hide" href="{$backLink}">{translate key="$backLinkLabel"}</a>
    {/if}
</div>
