...
<table width="100%" class="listing">
	...
	<tr class="heading" valign="bottom">
		<td width="3%">&nbsp;</td>
		<td width="15%">{sort_heading key="user.username" sort="username"}</td>
		<td width="30%">{sort_heading key="user.name" sort="name"}</td>
		<td width="20%">{sort_heading key="user.email" sort="email"}</td>
		<td width="28%" align="center">{translate key="common.action"}</td>
	</tr>
	{if $users->eof()}
	<tr valign="bottom">
		<td colspan="5" class="headseparator">&nbsp;</td>
...
{if $userExists}
	<p><input type="submit" value="{translate key="email.compose"}" class="button defaultButton"/>&nbsp;<input type="button" value="{translate key="common.selectAll"}" class="button" onclick="toggleChecked()" />  <input type="button" value="{translate key="common.cancel"}" class="button" onclick="document.location.href='{url page="manager" escape=false}'" />
	</p>
{/if}
</div>
</form>
...
<p class="u-mt-24">
    <a href="{url op="enrollSearch" path=$roleId}" class="link-button">{translate key="manager.people.enrollExistingUser"}</a> |
    ...
    <a href="{if $roleId}{url op="createUser" roleId=$roleId source=$enrollmentUrl}{else}{url op="createUser" source=$enrollmentUrl}{/if}" class="link-button">{translate key="manager.people.createUser"}</a> | <a href="{url op="enrollSyncSelect" path=$rolePath}" class="link-button">{translate key="manager.people.enrollSync"}</a>
</p>
...
<div class="colspan u-mb-0" id="colspan">	    
    ...
    <section class="u-display-flex u-justify-content-center">
        <div class="c-pagination">{page_links anchor="users" name="users" iterator=$users searchField=$searchField searchMatch=$searchMatch search=$search dateFromDay=$dateFromDay dateFromYear=$dateFromYear dateFromMonth=$dateFromMonth dateToDay=$dateToDay dateToYear=$dateToYear dateToMonth=$dateToMonth roleSymbolic=$roleSymbolic searchInitial=$searchInitial sort=$sort sortDirection=$sortDirection}
       </div>
    </section>
    {/if}
</div>
