	...
	<table width="100%" class="listing">
		...
		<div id="payment">
			...
			<tr>
				...
				<td>{$payment->getCurrencyCode()|escape} {$payment->getAmount()|number_format:2:'.':','}</td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td>
					{if $payment->getAssocDescription()}
						({$payment->getAssocId()|escape}) {$payment->getAssocDescription()|strip_unsafe_html|nl2br}</td>
					{else}
						-
					{/if}
			</tr>
...
<p><input type="button" value="{translate key="common.close"}" class="button" onclick="document.location.href='{url page="manager" op="viewPayments" escape=false}'" /></p>
