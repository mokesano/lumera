                    ...
                    <article class="u-full-height c-card c-card--flush" itemtype="http://schema.org/ScholarlyArticle">
                        ...
                        <div class="c-card__section c-meta">
                            ...
                            {if $article->getPubId('doi')}
                                <div class="u-hide c-meta__item c-meta__item--block-at-lg" data-test="info-DOI"><a title="Permanent link for {$article->getLocalizedTitle()|strip_tags|escape}" href="http://doi.org/{$article->getPubId('doi')|escape}">{$article->getPubId('doi')}</a></div>
                            {/if}
                                ...
                                {if $issueAvailable}
                                    <div class="u-hide c-meta__item c-meta__item--block-at-lg" data-test="galley">
                                        {if $galley->isPdfGalley()}
                                            <a class="pdf-galley" title="{$article->getLocalizedTitle()|strip_tags|escape}" href="{url journal=$journal->getPath() page="article" op="view" path=$publishedArticle->getBestArticleId($journal)}">{$galley->getGalleyLabel()|escape} <span class="fileSize">({$galley->getNiceFileSize()})</span> <span class="fileView">{$galley->getViews()} views</span></a>
                                        {elseif $galley->isHTMLGalley()}
                                            <a class="html-galley" title="{$article->getLocalizedTitle()|strip_tags|escape}" href="{url journal=$journal->getPath() page="article" op="view" path=$publishedArticle->getBestArticleId($journal)}">{$galley->getGalleyLabel()|escape} <span class="fileSize">({$galley->getNiceFileSize()})</span> <span class="fileView">{$galley->getViews()} views</span></a>
                                        {/if}
                                    </div>
                                {/if}
                            {/foreach}
	...
	<div class="colspan u-mb-0" id="colspan">	    
        ...
        {if $results->getPageCount() > 1}
            <section class="u-display-flex u-justify-content-center">
                <div class="c-pagination">{page_links anchor="results" iterator=$results name="search"}</div>
            </section>
	    {/if}
	</div>
    {/if}
