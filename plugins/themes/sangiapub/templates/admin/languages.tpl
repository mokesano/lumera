			...
			<tr valign="top">
				...
				<td>
					...
					{if $incompleteLocaleFound}
						<br/>
						<span class="formError">*</span>&nbsp;<span class="formErrorInfo">{translate key="admin.locale.maybeIncomplete"}</span>
					{/if}{* $incompleteLocaleFound *}
				</td>
			</tr>
		</table>
...
<form method="post" action="{url op="installLocale"}">
	...
	<div id="installNewLocales" class="block">
		...
		{if not $noLocalesToInstall}
			<p>
				<input type="submit" value="{translate key="admin.languages.installLocales"}" class="button defaultButton" />
				<input type="button" value="{translate key="common.cancel"}" class="button" onclick="document.location.href='{url page="admin" escape=false}'" />
			</p>
		{/if}
	</div>
