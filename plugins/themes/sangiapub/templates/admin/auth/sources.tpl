...
<div id="existingAuth" class="article block">
		...
		<div id="sources">
			<table width="100%" class="listing">
				...
				<tr valign="top">
					...
					<td colspan="1"><label for="defaultAuthId_0">{translate key="admin.auth.app"}</label></td>
	...
	<div id="createAuth" class="block">
		<h3 class="u-h2">{translate key="admin.auth.create"}</h3>
		<form method="post" action="{url op="createAuthSource"}">
			<input value="{$csrfToken|escape}" name="csrfToken" type="hidden">
			<p>{translate key="common.plugin"}:</p>
			<select name="plugin" size="1" class="selectMenu"><option value=""></option>{html_options options=$pluginOptions}</select>
			<input type="submit" value="{translate key="common.create"}" class="button" />
		</form>
	</div>
</div>
