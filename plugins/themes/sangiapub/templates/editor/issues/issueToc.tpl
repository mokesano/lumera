...
<form action="#">
	...
	<select name="issue" class="selectMenu" onchange="if(this.options[this.selectedIndex].value > 0) location.href='{url|escape:"javascript" op="issueToc" path="ISSUE_ID" escape=false}'.replace('ISSUE_ID', this.options[this.selectedIndex].value)" size="1">
		...
		{foreach from=$sections key=sectionKey item=section}
			...
			<table width="100%" class="listing" id="issueToc-{$sectionKey|escape}">
				...
				<tr id="article-{$article->getPublishedArticleId()|escape}" class="data">
					...
					<td class="drag">{if !$isLayoutEditor}<a href="{url op="submission" path=$articleId}" class="action">{/if}{$article->getLocalizedTitle()|strip_tags|truncate:60:"..."}{if !$isLayoutEditor}</a>{/if}{* [WIZDAM] Sudah di-batch (2 query total untuk SELURUH TOC ini, lihat ArticleType::attachDisplayLabels() di IssueManagementHandler::issueToc()) SEBELUM template ini dirender. Ditaruh di dalam kolom judul yang sudah ada (BUKAN kolom baru) supaya tidak perlu mengubah $numCols/colspan tabel ini. *}{if $article->getArticleTypeDisplayLabel()} <span class="articleTypeBadge">{$article->getArticleTypeDisplayLabel()|escape}</span>{/if}</td>
