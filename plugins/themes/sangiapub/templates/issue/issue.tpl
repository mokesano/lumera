...
{foreach name=sections from=$publishedArticles item=section key=sectionId}
	<section {if $section.title}id="{$section.title|escape|replace:" ":""}"{/if} class="u-mb-48 u-mt-48" {if $section.title}aria-labelledby="{$section.title|escape|replace:" ":""}"{/if} data-container-type="issue-section-list" data-track-component="issue section list">
		<div class="c-section-heading" data-test="title">
			{if $section.title}<h2><span class="content-break">{$section.title|escape}</span><span class="text-gray-light altSize u-hide">({$issue->getNumArticles()|escape} {translate key="article.articlesCount"})</span></h2>{/if}
		</div>
								...
								<div class="c-card__layout u-full-heights">
									{if $showCoverPage}
										<div class="c-card__image">
											<picture>
												...
												<img class="lazyload" loading="lazy" src="{$coverPagePath|escape}{$article->getLocalizedFileName()|escape}" alt="{$article->getLocalizedCoverPageAltText()|escape}" itemprop="image" />
											</picture>
										</div>
									{/if}
									...
									<div class="c-card__body u-display-flex u-flex-direction-column">
										...
										{if !(($article->getHideAuthor() != $smarty.const.AUTHOR_TOC_DEFAULT) || $article->getHideAuthor() == $smarty.const.AUTHOR_TOC_SHOW)}
											<ul class="c-author-list c-author-list--compact u-mt-auto" data-test="author-list">{assign var=authors value=$article->getAuthors()}{foreach from=$authors item=author name=authors}{assign var=fullname value=$author->getFullName()}{assign var=authorFirstName value=$author->getFirstName()}{assign var=authorMiddleName value=$author->getMiddleName()}{assign var=authorLastName value=$author->getLastName()}<li itemprop="creator" itemscope="name" itemtype="http://schema.org/Person"><span class="u-hide" itemprop="name">{$fullname|escape}</span>{if $authorFirstName !== $authorLastName}<span itemprop="given-name">{$authorFirstName}</span>{/if}{if $authorMiddleName}<span itemprop="middle-name">{$authorMiddleName}</span>{/if}{if $authorLastName}<span itemprop="surname">{$authorLastName}</span>{/if}</li>{/foreach}</ul>
										{/if}
									</div>
								</div>
