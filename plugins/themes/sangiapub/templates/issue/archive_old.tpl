...
<div class="content mb30 mq1200-padded position-relative">
    <section>
        <div class="pa40 cleared background-white">
            <ul id="volume-decade-list" class="clean-list ma0 clean-list grid-auto-fill medium-row-gap background-white" style="--column-width: 120px;">
        ...
        {if $issue->getYear() != $lastYear}
            ...
            {/if}
        <li>
            <div id="{$issue->getVolume()|escape}" class="content mq1200-padded volumes-detail">
                {assign var=lastYear value=$issue->getYear()}
                <h2 class="mb6 strong tighten-line-height text-gray volumes-detail__year">{$issue->getDatePublished('issue.firstYear')|date_format:"%Y"}
                    <span class="text-gray u-hide"> —</span>
                </h2>
                <a href="{native_url page="volume" volume=$issue->getVolume()}" data-track="click" data-track-action="view volume" data-track-label="link" class="volumes-detail__link">
    ...
    {/iterate}
        ...
        {* Menutup tag <ul>, </section>, </div> yang mungkin tertinggal *}
        {if $notFirstYear} {* Hanya tutup jika sudah pernah dibuka *}
                </section>
            </div>
        {/if}
