...
<div id="peerReview" class="block link">
	...
	<a href="{url op="selectReviewer" path=$submission->getId()}" class="link-button">{translate key="editor.article.selectReviewer"}</a>
	<a href="{url op="submissionRegrets" path=$submission->getId()}" class="link-button">{translate|escape key="sectionEditor.regrets.link"}</a>
