{**
 * templates/checkout/payment.tpl
 *
 * [WIZDAM EDITION]
 * Tahap 3: Payment (The Vault)
 * Multi-Gateway komplit (VA, QRIS, E-Wallet, CC Aktif, GPay, Manual).
 *}
{include file="common/header-parts/header-payment.tpl" pageTitle="checkout.payment.title"}
...
<div class="wizdam-checkout-wrapper">
    ...
    <div class="wizdam-checkout-container">
        ...
        <div class="wizdam-checkout-main">
            ...
            <form action="{url page="checkout" op="payment" path=$queuedPaymentId}" method="POST" id="execute-payment-form">
                    ...
                    <div class="payment-box" id="box-cc">
                        ...
                        <div class="payment-details">
                            ...
                            <button type="button" class="wizdam-btn-pay" onclick="executePayment('credit_card', this, event)">
                                Pay {$summary.currency} {$summary.grand_total|number_format:2}
                            </button>
                        </div>
                    </div>
        ...
        <div class="wizdam-checkout-sidebar">
            ...
            <div class="summary-row"><span>Subtotal</span><span>{$summary.currency} {$summary.subtotal|number_format:2}</span></div>
            
            {if $summary.discount > 0}
            <div class="summary-row" style="color: #28a745;"><span>Discount ({$summary.promo_code})</span><span>- {$summary.currency} {$summary.discount|number_format:2}</span></div>
            {/if}

            <div class="summary-row"><span>Tax ({$summary.tax_rate}%)</span><span>{$summary.currency} {$summary.tax_amount|number_format:2}</span></div>
            
            <div class="summary-total"><span>Total to pay</span><span>{$summary.currency} {$summary.grand_total|number_format:2}</span></div>
        </div>
