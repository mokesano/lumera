...
(function() {
    ...
    function initializeForm() {
        ...
        // NEW: Initialize change password validation
        setupChangePasswordValidation();
    }

    /**
     * Setup change password specific validation
     * @function
     * @returns {void}
     */
...
/**
 * PERSISTENT EARLY CAPTURE dengan multiple storage
 */
(function() {
    'use strict';
    ...
    function captureMessagesImmediately() {
        console.log('=== PERSISTENT EARLY CAPTURE ===');
        ...
        function tryCapture() {
            attempts++;
            console.log(`Persistent capture attempt ${attempts}`);
            ...
            passwordFields.forEach(function(fieldId) {
                ...
                console.log(`  ${fieldId}: error=${!!errorMessage}, success=${!!successMessage}`);
                ...
                if ((errorMessage || successMessage) && !window.earlyCapturedMessages.has(fieldId)) {
                    ...
                    console.log(`  ✅ Persistent captured ${fieldId}`);
                }
            });
            ...
            if (window.earlyCapturedMessages.size < 3 && attempts < maxAttempts) {
                ...
                console.log('=== PERSISTENT CAPTURE COMPLETE ===');
                console.log('Captured in Map:', window.earlyCapturedMessages.size);
                console.log('Captured in backup:', Object.keys(window.backupCapturedMessages).length);
            }
        }
...
/**
 * Enhanced transfer dengan fallback mechanisms
 */
function transferEarlyCapturedMessages() {
    console.log('=== ENHANCED TRANSFER ===');
    ...
    if (window.earlyCapturedMessages && window.earlyCapturedMessages.size > 0) {
        ...
        window.earlyCapturedMessages.forEach(function(capturedData, fieldId) {
            if (!originalMessages.has(fieldId)) {
                ...
                console.log(`✅ Transferred ${fieldId} from Map`);
            }
        });
        return;
    }
    
    // Fallback ke backup object
    if (window.backupCapturedMessages && Object.keys(window.backupCapturedMessages).length > 0) {
        console.log('Using backup object storage');
        Object.keys(window.backupCapturedMessages).forEach(function(fieldId) {
            if (!originalMessages.has(fieldId)) {
                ...
                console.log(`✅ Transferred ${fieldId} from backup`);
            }
        });
        return;
    }
...
/**
 * Show error when new password is same as old password
 * @function
 * @param {HTMLElement} newPasswordField - New password input element
 * @returns {void}
 */
function showPasswordSameAsOldError(newPasswordField) {
    console.log('=== showPasswordSameAsOldError called ===');
    
    // Force transfer dengan multiple attempts
    ...
    console.log('Flag set, checking originalMessages now:');
    const fieldKey = newPasswordField.id || newPasswordField.name;
    const storedMessages = originalMessages.get(fieldKey);
    console.log(`storedMessages for ${fieldKey}:`, storedMessages);
    
    // Panggil updateFieldMessages
    console.log('Calling updateFieldMessages...');
    updateFieldMessages(newPasswordField);
    console.log('updateFieldMessages completed');
}

/**
 * Clear error when new password is different from old password  
 * @function
 * @param {HTMLElement} newPasswordField - New password input element
 * @returns {void}
 */
...
// Test functions
window.checkAllStorages = function() {
    console.log('=== ALL STORAGE CHECK ===');
    console.log('earlyCapturedMessages Map:', window.earlyCapturedMessages);
    console.log('backupCapturedMessages Object:', window.backupCapturedMessages);
    console.log('originalMessages Map:');
    ...
    function updateFieldMessages(field) {
        ...
        if (field.type === 'password' && (field.name === 'password' || field.id === 'password' || 
            ...
            // If field has same-password error, keep error state and don't show other messages
            if (hasSamePasswordError) {
                // Transfer early captured messages jika perlu
                if (typeof transferEarlyCapturedMessages === 'function') {
                    transferEarlyCapturedMessages();
                }
                
                // Tampilkan error message seperti field lain
