...
function main() {
    try {
        ...
        return [
            ...
            'timestamp' => date('c'),
            'api_version' => 'v3.1.7' // Menunjukkan versi API
        ];
    }
}
...
function processOrcidData($orcid, $works_data, $person_data) {
    ...
    $result = [
        ...
        'personal_info' => [
            ...
            'data_source' => 'ORCID API' // Menunjukkan versi API
        ],
        'works' => [],
        'researcher_sdg_summary' => [],
        'status' => 'success',
    ...
    foreach ($researcher_sdg_summary as $sdg => $summary) {
        ...
        $researcher_sdg_contributions[$sdg] = [
            ...
            'contribution_level' => $isSignificantContributor ? 
                ($hasHighConfidenceWork ? 'Tinggi' : 'Sedang') : 'Rendah',
            'criteria_met' => [
...
function evaluateSDGContribution($text, $sdg) {
   ...
   if (isset($MEMORY_CACHE[$cacheKey])) {
    ...
    // Gabungkan substantive dan causal ke dalam satu skor
    $combinedSubstantiveScore = ($substantiveScore * 0.6) + ($causalScore * 0.4);
    
    // Adaptif: Sesuaikan bobot berdasarkan ketersediaan teks
    $weights = $CONFIG;
    
    // Analisis teks berdasarkan panjang dan karakteristik
    $hasAbstract = strlen($text) > 200; // Asumsi teks > 200 karakter memiliki abstrak
    $hasTitleOnly = strlen($text) < 100; // Teks sangat pendek, kemungkinan hanya judul
    
    // Jika teks sangat pendek (seperti hanya judul tanpa abstrak)
    if ($hasTitleOnly) {
        $weights['KEYWORD_WEIGHT'] = 0.45;      // Distribusi yang direvisi
        $weights['SIMILARITY_WEIGHT'] = 0.40;
        $weights['SUBSTANTIVE_WEIGHT'] = 0.15;
        
        // Heuristic bonus: Jika judul pendek mengandung indikator substantif kuat
        if (containsStrongSubstantiveIndicator($text, $sdg)) {
            $combinedSubstantiveScore = max($combinedSubstantiveScore, 0.3); // Minimal score 0.3
        }
    }
    // Jika teks sedang tapi belum cukup panjang untuk dianggap memiliki abstrak lengkap
    else if (!$hasAbstract) {
        $weights['KEYWORD_WEIGHT'] = 0.40;
        $weights['SIMILARITY_WEIGHT'] = 0.40;
        $weights['SUBSTANTIVE_WEIGHT'] = 0.20;
    }
    // Jika teks lengkap dengan abstrak, gunakan bobot default dari CONFIG
    
    // Kombinasikan skor dengan pembobotan
    $combinedScore = (
        ($keywordScore * $weights['KEYWORD_WEIGHT']) +
        ($similarityScore * $weights['SIMILARITY_WEIGHT']) +
        ($combinedSubstantiveScore * $weights['SUBSTANTIVE_WEIGHT'])
    );
   ...
   $result = [
    ...
    // Tambahkan informasi mengenai bobot yang digunakan
    $result['weights_used'] = [
        'keyword' => $weights['KEYWORD_WEIGHT'],
        'similarity' => $weights['SIMILARITY_WEIGHT'],
        'substantive' => $weights['SUBSTANTIVE_WEIGHT'],
...
/**
 * Mendeteksi indikator substantif yang kuat dalam judul pendek
 */
function containsStrongSubstantiveIndicator($text, $sdg) {
    // Kata kerja dan frasa yang menunjukkan kontribusi substantif
    $substantiveIndicators = [
        // Kata kerja tindakan
        'improve', 'enhance', 'develop', 'reduce', 'increase', 'strengthen',
        'meningkatkan', 'memperbaiki', 'mengembangkan', 'mengurangi', 'memperkuat',
        
        // Frasa kontributif
        'framework for', 'approach to', 'method for', 'strategy for', 'solution to',
        'kerangka untuk', 'pendekatan untuk', 'metode untuk', 'strategi untuk',
        
        // Frasa penelitian
        'analysis of', 'study of', 'research on', 'investigation of', 'assessment of',
        'analisis', 'studi', 'penelitian', 'investigasi', 'penilaian'
    ];
    
    // Cek indikator umum
    foreach ($substantiveIndicators as $indicator) {
        if (stripos($text, $indicator) !== false) {
            return true;
        }
    }
    
    // Indikator spesifik per SDG
    $sdgSpecificIndicators = [
        $sdgSpecificIndicators = [
    ...
    // Cek indikator spesifik SDG jika tersedia
    if (isset($sdgSpecificIndicators[$sdg])) {
        foreach ($sdgSpecificIndicators[$sdg] as $indicator) {
            if (stripos($text, $indicator) !== false) {
                return true;
            }
        }
...
// ==============================================
/**
* Ekstrak nama peneliti dari data ORCID, penanganan error yang ditingkatkan
