...
$CONFIG = [
    ...
    'MAX_SDGS_PER_WORK' => 5,          // Jumlah maksimum SDG per karya
    ...
    'ACTIVE_CONTRIBUTOR_THRESHOLD' => 0.50,  // Threshold kontributor aktif
    'RELEVANT_CONTRIBUTOR_THRESHOLD' => 0.35, // Threshold kontributor relevan
    'DISCUSSANT_THRESHOLD' => 0.25,    // Threshold diskutor
...
function main() {
   try {
       ...
       return [
           ...
           'timestamp' => date('c'),
           'api_version' => 'v4.1.7' // Menunjukkan versi API
       ];
   }
}

// [Sisipkan handleOrcidRequest dan handleDoiRequest]
...
// ==============================================
// V4: FUNGSI EVALUASI ORIENTASI DAMPAK
// ==============================================
...
function determineContributorType($combinedScore, $causalScore, $impactScore) {
   ...
   if ($contributionScore >= $CONFIG['ACTIVE_CONTRIBUTOR_THRESHOLD'] && 
       ...
       return [
           'type' => 'Kontributor Aktif',
           'description' => 'Penelitian dengan kontribusi substantif terhadap SDG',
           'score' => round($contributionScore, 3)
       ];
   } elseif ($contributionScore >= $CONFIG['RELEVANT_CONTRIBUTOR_THRESHOLD']) {
       return [
           'type' => 'Kontributor Relevan',
           'description' => 'Penelitian dengan relevansi jelas terhadap SDG',
           'score' => round($contributionScore, 3)
       ];
   } elseif ($contributionScore >= $CONFIG['DISCUSSANT_THRESHOLD']) {
       return [
           'type' => 'Diskutor',
           'description' => 'Penelitian membahas tema terkait SDG tanpa kontribusi jelas',
           'score' => round($contributionScore, 3)
       ];
   } else {
       return [
           'type' => 'Tidak Relevan',
           'description' => 'Penelitian tidak menunjukkan relevansi yang cukup dengan SDG',
           'score' => round($contributionScore, 3)
       ];
   }
}
...
// ==============================================
// V4: MODIFIKASI FUNGSI PROCESS DATA
// ==============================================
...
/**
* Processes DOI data to create a article profile with SDG analysis
* Improved to ensure proper data structure display
*/
function processDoiData($doi, $data) {
   ...
   foreach ($allSdgs as $sdg) {
       ...
       if ($matched) {
           // PERUBAHAN: Gunakan variabel berbeda untuk hasil evaluasi
   ...
   // PERUBAHAN: Buat struktur respons final
   $finalResult = [
       'doi' => $doi,
       'title' => $title,
       'abstract' => $abstract,
...
/**
 * Processes ORCID data to create a structured researcher profile with SDG analysis
 * Improved to ensure proper data structure display with analysis inside works array
 */
function processOrcidData($orcid, $works_data, $person_data) {
    global $SDG_KEYWORDS, $CONFIG;

    // Ekstrak informasi personal - tambahkan debugging
    $name = extractOrcidName($person_data);
    $institutions = extractOrcidInstitutions($person_data);
    
    // Debug: jika kosong, cek struktur data person
    if (empty($name)) {
        error_log("ORCID Profile Debug - Name Empty. Person data structure: " . print_r($person_data, true));
        // Fallback: gunakan ORCID sebagai nama jika tidak bisa diekstrak
        $name = "Peneliti " . $orcid;
    }
    ...
    $result = array(
        ...
        'personal_info' => array(
            ...
            'data_source' => !empty($person_data) ? 'ORCID API' : 'Not available',
            'retrieval_status' => !empty($person_data) ? 'OK' : 'Data profil tidak tersedia'
        ),
        'works' => array(),
        'researcher_sdg_summary' => array(),
        'contributor_profile' => array(), // V4: Tambahkan profil kontributor
        'status' => 'success',
        'api_version' => 'v4.1.7',
        'timestamp' => date('c')
    );
    ...
    if (isset($works_data['group']) && is_array($works_data['group'])) {
        foreach ($works_data['group'] as $work) {
            ...
            foreach ($SDG_KEYWORDS as $sdg => $keywords) {
                ...
                if ($matched) {
                    $analysisResult = evaluateSDGContribution($preprocessedText, $sdg);
                    
                    // Hanya simpan SDG dengan skor minimal
                    if ($analysisResult['score'] > $CONFIG['MIN_SCORE_THRESHOLD']) {
                        $sdgAnalysis[$sdg] = $analysisResult;
                    }
                }
            }
            ...
            // Buat struktur karya yang tepat dengan semua analisis di dalamnya
            $workEntry = [
                'title' => $title,
                'doi' => $doi,
                'abstract' => substr($abstract, 0, 300) . (strlen($abstract) > 300 ? '...' : ''),
                'sdgs' => $filteredSdgs,
                'sdg_confidence' => $sdgConfidence,
                'contributor_types' => $sdgContributorTypes,
            ...
            // PENTING: Tambahkan detailed_analysis ke dalam struktur karya
            // sehingga semua analisis berada di dalam works, bukan di luar
            $workEntry['detailed_analysis'] = $sdgAnalysis;
            
            // Tambahkan karya ke daftar works
            $processed_works[] = $workEntry;
            
            // Update statistik agregat untuk ringkasan peneliti
            foreach ($sdgAnalysis as $sdg => $analysis) {
                if ($analysis['score'] >= $CONFIG['CONFIDENCE_THRESHOLD']) {
    ...
    // V4: Buat profil kontributor SDG
    $contributorProfile = [];
    foreach ($researcher_sdg_summary as $sdg => $summary) {
        // Tentukan tipe kontributor berdasarkan distribusi
        $activeCount = $summary['contributor_types']['Kontributor Aktif'];
        $relevantCount = $summary['contributor_types']['Kontributor Relevan'];
        $discussantCount = $summary['contributor_types']['Diskutor'];
        $totalWorks = $summary['work_count'];
        
        // Tentukan tipe kontributor dominan
        $dominantType = 'Diskutor'; // Default
        if ($activeCount / $totalWorks >= 0.3) {
            $dominantType = 'Kontributor Aktif';
        } elseif (($activeCount + $relevantCount) / $totalWorks >= 0.5) {
            $dominantType = 'Kontributor Relevan';
        }
    ...
    // Masukkan hasil analisis ke dalam struktur result
    $result['works'] = $processed_works;
    $result['researcher_sdg_summary'] = $researcher_sdg_summary;
    $result['contributor_profile'] = $contributorProfile;
...
function determineContributionStrength($summary) {
   ...
   $activeRatio = $summary['contributor_types']['Kontributor Aktif'] / max(1, $summary['work_count']);
   ...
   if ($score >= 10) return 'Sangat Kuat';
   elseif ($score >= 7) return 'Kuat';
   elseif ($score >= 4) return 'Moderat';
   else return 'Rendah';
}

// ==============================================
// V4: FUNGSI DETEKSI KAUSAL YANG DITINGKATKAN
// ==============================================
...
/**
 * Model kombinasi multifaktor untuk evaluasi SDG - versiperbaikan V4
 */
...
/**
 * Ekstrak institusi peneliti dari data ORCID dengan penanganan error
 */
...
/**
 * Fungsi untuk mengambil data ORCID Person dengan penanganan error yang ditingkatkan
 */
