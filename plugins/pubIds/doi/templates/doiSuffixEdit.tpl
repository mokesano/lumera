...
{if $enableObjectDoi}
	...
	<div id="pub-id::doi">
		...
		{if !$excludeDoi}
			...
			{else}
				<p class="u-mb-16 bold">
				    <span class="action">{$pubIdPlugin->getPubId($pubObject, true)|escape}</span>
				</p>
				{capture assign=translatedObjectType}{translate key="plugins.pubIds.doi.editor.doiObjectType"|cat:$pubObjectType}{/capture}
				<p class="u-mb-16 u-mt-16">
				    <span class="instruct">{translate key="plugins.pubIds.doi.editor.doiNotYetGenerated" pubObjectType=$translatedObjectType}</span>
				</p>
			{/if}
		{/if}
		...
		<p class="u-mb-24">
    		...
    		{capture assign=translatedObjectType}{translate key="plugins.pubIds.doi.editor.doiObjectType"|cat:$pubObjectType}{/capture}
    		<span>{translate key="plugins.pubIds.doi.editor.excludePubObject" pubObjectType=$translatedObjectType}</span>
		</p>
	</div>
	...
	{if $enableArticleDoi || $enableGalleyDoi || $enableSuppFileDoi}
		<div id="pub-id::doi::excludeIssueObjects">
			...
			<section class="u-mb-24">
			    <p><span class="instruct">{translate key="plugins.pubIds.doi.editor.excludeIssueObjectsDoi.description"}</span></p>
			    <input type="submit" name="excludeIssueObjects_{$pubIdPlugin->getPubIdType()|escape}" value="{translate key="plugins.pubIds.doi.editor.excludeIssueObjectsDoi"}" class="action" />
			</section>
			<section class="u-mb-24">
    			<p><span class="instruct">{translate key="plugins.pubIds.doi.editor.clearIssueObjectsDoi.description"}</span></p>
    			<input type="submit" name="clearIssueObjects_{$pubIdPlugin->getPubIdType()|escape}" value="{translate key="plugins.pubIds.doi.editor.clearIssueObjectsDoi"}" class="action" />
			</section>
		</div>
		{assign var=divSeparator value=true}
	{/if}
