...
function displayArticleHighlights() {
  try {
    ...
    if (creditElement) {
      creditElement.textContent = 'Generate by Wizdam AI';
    }
    ...
    const addLinkWithLoading = (referenceLinks, href, text, svgIcon) => {
        ...
        let link = $(text === 'View PDF'
            ? `<a class="anchor link anchor-primary" href="${href}" target="_blank" aria-label="${text}">
                <span class="anchor-text-container">${svgIcon}<span class="anchor-text view-pdf">${text}</span></span></a>`
            : `<a class="anchor link anchor-primary" href="${href}" target="_blank" aria-label="${text}">
        ...
        $('.reference').each(function() {
            ...
            if (httpMatches) {
                httpMatches.forEach(httpUrl => {
                    let isDoiHost = false;
                    try {
                        const parsedUrl = new URL(/^https?:\/\//i.test(httpUrl) ? httpUrl : 'https://' + httpUrl);
                        const host = parsedUrl.hostname.toLowerCase();
                        isDoiHost = host === 'doi.org' || host.endsWith('.doi.org');
                    } catch (e) {
                        isDoiHost = false;
                    }
