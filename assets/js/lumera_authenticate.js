...
(function() {
    ...
    /**
     * Read minimum password length FROM SERVER (hidden input).
     * Stores the value in window.__minPasswordLength if valid.
     * If absent or invalid, no minimum length is enforced.
     */
    function readMinPasswordLength() {
        var input = document.querySelector('input[name="authLength"]');
        if (input) {
            var val = parseInt(input.value, 10);
            if (!isNaN(val) && val > 0) {
                window.__minPasswordLength = val;
            }
        }
    }

    /**
     * Helper: check if password meets the server‑configured minimum length.
     * @param {string} password - Password value
     * @returns {boolean} true if no minimum is set or length is sufficient
     */
    function isPasswordLengthMet(password) {
        if (window.__minPasswordLength === undefined) return true;
        return password.length >= window.__minPasswordLength;
    }
    ...
    function initializeForm() {
        readMinPasswordLength(); // From config
        setupFloatingLabels();
        setupPasswordToggle();
    ...
    function getUsernameInfo() {
        // Try common Lumera username field selectors in priority order
        const usernameField =
     ...
     * Strength levels determined by mandatory requirements and optional username difference.
     * Mandatory requirements include dynamic minimum length from server.
     * @function
    ...
    function updatePasswordStrengthForField(passwordInput, fieldConfig) {
        ...
        // Length requirement now uses server-provided minimum (or true if not set)
        const mandatoryReqs = [
            isPasswordLengthMet(password),   // dynamic length
     ...
     * Uses dynamic minimum password length from server.
     * @function
    ...
    function updateFieldMessages(field) {
        ...
        if (isPasswordField) {
            ...
            if (isRequired && !value && isTouched && !isFocused && storedMessages && storedMessages.errorElement) {
                ...
                const mandatoryReqs = [
                    isPasswordLengthMet(value),   // menggunakan konfigurasi server
                    /[A-Z]/.test(value),
                    /[a-z]/.test(value),
                    /[0-9]/.test(value),
        ...
        else if (value && !field.classList.contains('error') && storedMessages && storedMessages.successElement) {
            const successClone = storedMessages.successElement.cloneNode(true);
            insertMessageElement(fieldContainer, successClone);
            field.classList.add('success');
            field.classList.remove('error');
     ...
     * Uses dynamic password length from server.
     * @function
     * @param {HTMLElement} field - The form field
    ...
    function validateField(field) {
        ...
        // Password validation uses 5 mandatory requirements (length from server)
        if (field.type === 'password' && value) {
            const mandatoryReqs = [
                isPasswordLengthMet(value),  // dinamis
                /[A-Z]/.test(value),
                /[a-z]/.test(value),
                /[0-9]/.test(value),
                /[^A-Za-z0-9]/.test(value)
     ...
     * Requirements include dynamic length from server.
     * @function
     * @returns {void}
     */
     ...
     * Uses dynamic length from server.
     * @function
    ...
    function updatePasswordRequirements(password) {
        ...
        if (!usernameInfo.exists) {
            ...
            usernameReqMet = false;
        } else {
            const pwLower = password.toLowerCase();
            const unLower = usernameInfo.value;
            if (unLower.length >= 3) {
        ...
        // Panjang minimum dari server (jika tidak ada, true)
        const requirements = [
            { id: 'req-length',    test: isPasswordLengthMet(password) },
    ...
    function initializeTurnstile() {
        if (typeof turnstile === 'undefined' || !AuthForms.config.siteKey) return;
        const container = document.getElementById('turnstile-container');
        const loading = document.getElementById('turnstile-loading');
        const error = document.getElementById('turnstile-error');
        if (!container || !loading) return;
        if (!loading.parentNode) container.parentNode.appendChild(loading);
        try {
            turnstile.render(container, {
                ...
                'error-callback': function() {
                    if (loading && loading.parentNode) loading.parentNode.removeChild(loading);
                    if (error) {
                        if (!error.parentNode) container.parentNode.appendChild(error);
    ...
    function initializeRememberMeState(checkbox) {
        try {
            ...
            console.warn('[Wizdam Auth] Could not access localStorage');
        }
    }
    ...
    function handleEnhancedRememberMeChange(checkbox) {
        ...
        const customEvent = new CustomEvent('ojsRememberMeChanged', {
            detail: { checked: checkbox.checked, timestamp: new Date().toISOString() }
        });
        document.dispatchEvent(customEvent);
        if (typeof gtag !== 'undefined') {
            gtag('event', 'remember_me_toggle', { 'value': checkbox.checked ? 1 : 0 });
        }
    }
