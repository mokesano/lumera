  ...
  'use strict';
    ...
    function displayArticleHighlights() {
      try {
        ...
        // Buat daftar highlight dengan DOM API agar teks tidak diinterpretasikan sebagai HTML
        ...
        $('.reference').each(function() {
            ...
            if (httpMatches) {
                httpMatches.forEach(httpUrl => {
                    let isDoiHost = false;
                    try {
                        const parsedUrl = new URL(httpUrl.startsWith('www.') ? 'https://' + httpUrl : httpUrl);
                        const hostname = parsedUrl.hostname.toLowerCase();
                        isDoiHost = hostname === 'doi.org' || hostname.endsWith('.doi.org');
                    } catch (e) {
                        isDoiHost = false;
                    }
