...
// [SHIM] Tangkap request asset gambar (skema semantik & kompatibilitas)
import('lib.wizdam.image.AssetDispatcher');
$pathInfo = isset($_SERVER['PATH_INFO']) ? (string) $_SERVER['PATH_INFO'] : '';
if ($pathInfo !== '' && AssetDispatcher::dispatch($pathInfo)) {
    exit; // Jaga-jaga; dispatch() sendiri sudah exit().
}
