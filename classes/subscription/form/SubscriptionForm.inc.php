...
class SubscriptionForm extends Form {
    ...
    public function SubscriptionForm($subscriptionId = null, $userId = null) {
        trigger_error(
            "Class '" . get_class($this) . "' uses deprecated constructor parent::SubscriptionForm(). Please refactor to use parent::__construct().",
            E_USER_DEPRECATED
        );
        // Note: Since SubscriptionForm is now abstract-ish (expects template from child),
        // direct instantiation might fail if template isn't handled.
        // Assuming default template for shim if direct call (unlikely for base class):
        self::__construct('subscription/subscriptionForm.tpl', $subscriptionId, $userId);
    }
    ...
    public function initData() {
        ...
        // return parent::initData(); // [WIZDAM NOTE] Form::initData usually void, removing return
        parent::initData();
    }
    ...
    public function readInputData() {
        ...
        if (!$nonExpiring) {
            ...
            $this->addCheck(new FormValidatorCustom($this, 'dateEndYear', 'required', 'manager.subscriptions.form.dateEndValid', function($dateEndYear) {
                $minYear = date('Y') + SUBSCRIPTION_YEAR_OFFSET_PAST;
                $maxYear = date('Y') + SUBSCRIPTION_YEAR_OFFSET_FUTURE;
                return ($dateEndYear >= $minYear && $dateEndYear <= $maxYear) ? true : false;
            }));
        ...
        if ($this->_data['notifyEmail'] == 1) {
            $this->addCheck(new FormValidatorCustom($this, 'notifyEmail', 'required', 'manager.subscriptions.form.subscriptionContactRequired', function() {
                ...
                return $subscriptionName != '' && $subscriptionEmail != '' ? true : false;
            }, array()));
        }
    }
