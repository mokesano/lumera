...
class Upgrade extends Installer {
     ...
     * @param bool $isPlugin
     */
    public function __construct(array $params, string $installFile = 'upgrade.xml', bool $isPlugin = false) {
    ...
    public function migrateIssueLabelAndSettings(): bool {
        ...
        while (!$result->EOF) {
            ...
            foreach ($settings as $name => $value) {
                $journalDao->update(
                    'INSERT INTO journal_settings (journal_id, setting_name, setting_value, setting_type) VALUES (?, ?, ?, ?)',
                    array((int) $row['journal_id'], $name, $value ? 1 : 0, 'bool')
                );
            }
            $result->MoveNext();
        }
     ...
     * @return bool
     */
    public function clearDataCache(): bool {
     ...
     * @return bool
     */
    public function migratePublisher(): bool {
        $journalSettingsDao = DAORegistry::getDAO('JournalSettingsDAO');
        ...
        while (!$result->EOF) {
            $row = $result->GetRowAssoc(false);
            $publisher = null;
            // WIZDAM NOTE: @unserialize is unsafe but necessary for upgrading old data
            ...
            foreach (array('note' => 'publisherNote', 'institution' => 'publisherInstitution', 'url' => 'publisherUrl') as $old => $new) {
                if (isset($publisher[$old])) {
                    $journalSettingsDao->update(
        ...
        $journalSettingsDao->update('DELETE FROM journal_settings WHERE setting_name = ?', array('publisher'));
     ...
     * @return bool
     */
    public function setGalleyLocales(): bool {
     ...
     * @return bool
     */
    public function migrateUserSettings(): bool {
        $userSettingsDao = DAORegistry::getDAO('UserSettingsDAO');

        $result = $userSettingsDao->retrieve('SELECT u.user_id, u.setting_name, u.setting_value, u.setting_type, s.primary_locale, u.journal_id FROM user_settings_old u, site s');
        while (!$result->EOF) {
            $row = $result->GetRowAssoc(false);
            $userSettingsDao->update('INSERT INTO user_settings (user_id, setting_name, assoc_id, setting_value, setting_type, locale) VALUES (?, ?, ?, ?, ?, ?)', array((int) $row['user_id'], $row['setting_name'], (int) $row['journal_id'], $row['setting_value'], $row['setting_type'], $row['primary_locale']));
            $result->MoveNext();
        }
        $result->Close();
        unset($result);
    ...
    public function separateSubscriptions(): bool {
        ...
        while (!$result->EOF) {
            ...
            if ($row['institutional']) {
                ...
                // [WIZDAM FIX] Replaced deprecated each() with foreach
     ...
     * @return bool
     */
    public function cleanImageAlts(): bool {
        $imageSettings = array(
        ...
        while ($journal = $journals->next()) {
            foreach ($imageSettings as $imageSettingName => $newSettingName) {
                ...
                $newSetting = array();
                if ($imageSetting) {
                    foreach ($imageSetting as $locale => $setting) {
                        if (isset($setting['altText'])) {
                            $newSetting[$locale] = $setting['altText'];
                        }
                    }
                }
                if (!empty($newSetting)) {
    ...
    public function migrateReviewingInterests(): bool {
        ...
        $userDao->update('DELETE FROM user_settings WHERE setting_name = ?', array('interests'));
        ...
        while (!$result->EOF) {
            ...
            $notificationDao->update(
                sprintf(
                    ...
                    VALUES
                        (?, ?, %s, %s, ?, ?, ?, ?)',
                    $notificationDao->datetimeToDB($notification->getDateCreated()),
                    $notificationDao->datetimeToDB($notification->getDateRead())
                ),
                array(
    ...
    public function migrateReviewingInterests2(): bool {
        ...
        if ($idempotenceCheck->RecordCount() == 1 && $row['assoc_id'] == 0 && $row['assoc_type'] == 0) {
            return true;
        }
    ...
    public function migrateTimedViewsUsageStatistics(): bool {
        ...
        $tempStatsDao->update(
            ...
            GROUP BY tr.assoc_type, tr.assoc_id, tr.day, tr.country_id, tr.region, tr.city, tr.file_type, tr.load_id, a.journal_id, pa.issue_id',
            $params
        );
        ...
        $tempStatsDao->update(
            ...
            WHERE tr.load_id = ? AND tr.assoc_type = ?
            GROUP BY tr.assoc_type, tr.assoc_id, tr.day, tr.country_id, tr.region, tr.city, tr.file_type, tr.load_id, ag.article_id, a.journal_id, pa.issue_id',
            $params
        );
    ...
    public function removeCustomIdentifierSuffixOption(): bool {
        ...
        while ($journal = $journals->next()) {
            ...
            if ($urnSuffixSetting == 'publisherId') {
    ...
    public function syncArticleIdentifiers($installer, $version) {
        ...
        // 3. Looping seluruh artikel di database
        while (!$result->EOF) {
            $row = $result->GetRowAssoc(false);
            $articleId = (int) $row['article_id'];
            
            // 4. Eksekusi Orchestrator langsung dari DAO!
            // Mengecek, mengaudit, dan men-generate identifier secara otomatis.
            $articleDao->getArticleIdentifiers($articleId);
