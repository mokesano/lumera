...
$.extend($.validator, {
	...
	methods: {
		...
		email: function(value, element) {
			...
			// Fast structural checks to avoid catastrophic backtracking from a monolithic regex.
			...
			// Common local-part characters (dot allowed but not at ends or consecutively).
			if (!/^[A-Z0-9!#$%&'*+\/=?^_`{|}~.-]+$/i.test(localPart) ||
				/^\./.test(localPart) || /\.$/.test(localPart) || /\.\./.test(localPart)) {
				return false;
			}

			// Require a dotted domain; validate labels separately.
			...
			for (var i = 0; i < labels.length; i++) {
				...
				if (!/^[A-Z0-9-]+$/i.test(label) || label.charAt(0) === "-" || label.charAt(label.length - 1) === "-") {
					return false;
				}
			}
