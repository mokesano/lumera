...
(function($) {
	$.fn.splitter = function(args) {
		...
		function resolvePaneTarget(target) {
			if (!target) {
				return $();
			}
			if (target.jquery) {
			...
			if (typeof target === 'string') {
				// Use selector-only lookup to avoid jQuery HTML interpretation.
				return $($.find(target));
			}
			return $();
		}
		...
		return this.each(function() {
			...
			var A = resolvePaneTarget(args.A); // left/top frame
			var B = resolvePaneTarget(args.B); // right/bottom frame
