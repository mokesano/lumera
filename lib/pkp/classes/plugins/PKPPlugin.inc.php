...
class PKPPlugin {
     ...
     * @return boolean
     */
    public function addLocaleData($locale = null): bool {
     ...
     */
    public function getContextSpecificSetting(array $context, string $name) {
    ...
    public function getSettingMainContext($request = null): array {
        ...
        if ($contextDepth > 0) {
            if ($this->isSitePlugin()) {
                ...
                // Metode 2: Fallback via Request object
                if (!$mainContext && $request) {
                    $mainContext = $request->getJournal();
                }
    
                // Metode 3: Fallback via static Request (legacy)
                if (!$mainContext) {
                    $mainContext = Request::getJournal();
                }
    
                // Metode 4: Fallback via Registry langsung
                if (!$mainContext) {
                    $mainContext = Registry::get('currentJournal');
                }
            }
    
            if ($mainContext) {
                $settingContext[] = $mainContext->getId();
            }
