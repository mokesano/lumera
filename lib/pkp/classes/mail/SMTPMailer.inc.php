...
class SMTPMailer {
    ...
    /** @var string SMTP server hostname (default localhost) */
    public $server;

    /** @var string|int SMTP server port (default 25) */
    public $port;

    /** @var string Authentication mechanism (optional) (PLAIN | LOGIN | CRAM-MD5 | DIGEST-MD5) */
    public $auth;

    /** @var string Username for authentication (optional) */
    public $username;

    /** @var string Password for authentication (optional) */
    public $password;

    /** @var resource|bool|null SMTP socket */
    public $socket;
     ...
     * @return boolean
     */
    public function mail($mail, $recipients, $subject, $body, $headers = '') {
    ...
    /**
     * Receive a response.
     * @param string|array $expected expected response code(s)
     * @return boolean
     */
    public function receive($expected) {
        $data = '';
     ...
     * @param string|array $expected expected response code
     * @param string $data buffer
     * @return boolean
     */
    public function receiveData($expected, &$data) {
        do {
     ...
     * @return boolean
     */
    public function authenticate_plain() {
     ...
     * @return boolean
     */
    public function authenticate_cram_md5() {
     ...
     * @return boolean
     */
    public function authenticate_digest_md5() {
        ...
        while(!empty($data)) {
            // Using modern destructuring or keep explode/list if simple, but suppressing error
            $parts = explode('=', $data, 2);
            $key = $parts[0];
            $rest = $parts[1] ?? '';
            
            if ($rest[0] != '"') {
                $subParts = explode(',', $rest, 2);
                $value = $subParts[0];
                $data = $subParts[1] ?? '';
            } else {
                $subParts = explode('"', substr($rest, 1), 2);
                $value = $subParts[0];
                $data = isset($subParts[1]) ? substr($subParts[1], 1) : '';
            }
            if (!empty($value))
                $challenge[$key] = $value;
        }
        ...
        $cnonce = md5(uniqid((string) mt_rand(), true));
     ...
     * @param string $hashfn e.g., 'md5' or 'sha1'
     * @param int $blocksize
     * @param string $data
     * @param string $key
     * @return string (as hex)
     */
    public function hmac($hashfn, $blocksize, $data, $key) {
