...
class CitationForm extends Form {
    ...
    protected DataObject $_assocObject;

    /** @var bool */
    protected bool $_unsavedChanges = false;

    /** @var array all properties contained in the citation */
    protected array $_citationProperties = [];

    /** @var array identifiers for form fields */
    protected array $_citationFormFieldNames = [];
     ...
     */
    protected array $_metadataDescriptions = [];
     ...
     */
    public function __construct($request, $citation, $assocObject) {
        parent::__construct(CITATION_FORM_FULL_TEMPLATE); // Assuming parent takes template, if not, strict parent::__construct()
        
        assert($citation instanceof Citation);
        assert($assocObject instanceof DataObject);

        $this->_citation = $citation;
        $this->_assocObject = $assocObject;

        // Identify all form field names for the citation
        $this->_citationFormFieldNames = [];
     ...
     */
    public function CitationForm($request, $citation, $assocObject) {
        ...
        self::__construct($request, $citation, $assocObject);
    }
     ...
     * @param bool $unsavedChanges
     * @return bool
     */
    public function setUnsavedChanges($unsavedChanges) {
        return $this->_unsavedChanges = (bool)$unsavedChanges;
    }
    ...
    public function readInputData() {
        $this->readUserVars(['rawCitation', 'citationFilters', 'citationState', 'citationApproved']);
    ...
    public function validate() {
        ...
        foreach($metadataSchemas as $metadataSchema) { /* @var $metadataSchema MetadataSchema */
            ...
            foreach($metadataSchema->getProperties() as $propertyName => $property) {
                $fieldName = $metadataSchema->getNamespacedPropertyId($propertyName);
                $fieldValue = trim((string)$this->getData($fieldName));
                ...
                if (empty($fieldValue)) {
                    ...
                    switch(true) {
                        ...
                        case isset($allowedTypes[METADATA_PROPERTY_TYPE_INTEGER]) && is_numeric($fieldValue):
                            $typedFieldValues = [(int)$fieldValue];
                            break;
     ...
     * @return string
     */
    public function fetch($request, $template = CITATION_FORM_FULL_TEMPLATE, $display = false) {
        ...
        if ($citation->getId()) {
            ...
            // Avoid modifying the internal array directly if we want to preserve state, 
            // but array_pop is likely intended behavior here.
            ...
            $availableParserFilters = $citationDao->getCitationFilterInstances($context->getId(), CITATION_PARSER_FILTER_GROUP, [], true);
            $templateMgr->assign('availableParserFilters', $availableParserFilters);
            $availableLookupFilters = $citationDao->getCitationFilterInstances($context->getId(), CITATION_LOOKUP_FILTER_GROUP, [], true);
        ...
        if ($template == CITATION_FORM_FULL_TEMPLATE) {
            ...
            foreach($sourceDescriptions as $sourceDescription) {
                ...
                foreach ($sourceDescription->getStatements() as $propertyName => $value) {
                    ...
                    $thisDescription[$sourcePropertyId] = [
            ...
            $citationApproved = ($citation->getCitationState() == CITATION_APPROVED);
     ...
     * @param MetadataProperty $property
     * @param mixed $value
     * @return string
     */
    protected function _getStringValueFromMetadataStatement($property, $value): string {
     ...
     */
    protected function _orderByDisplayName(array $fieldList): array {
        uasort($fieldList, fn($a, $b) => strcmp($a['displayName'], $b['displayName']));
        return $fieldList;
    }
}
?>