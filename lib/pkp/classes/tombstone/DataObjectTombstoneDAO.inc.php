...
class DataObjectTombstoneDAO extends DAO {
     ...
     */
    public function DataObjectTombstoneDAO() {
        trigger_error(
            "Class '" . get_class($this) . "' uses deprecated constructor parent::DataObjectTombstoneDAO(). Please refactor to use parent::__construct().",
            E_USER_DEPRECATED
        );
        self::__construct();
    }
    ...
    public function updateObject($dataObjectTombstone) {
        // WIZDAM FIX: Replaced undefined $publicationFormatTombstone with $dataObjectTombstone
     ...
     * @return int
     */
    public function getInsertTombstoneId() {
    ...
    public function insertOAISetObjects($dataObjectTombstone) {
        $ids = $dataObjectTombstone->getOAISetObjectsIds();
        if (is_array($ids)) {
            foreach ($ids as $assocType => $assocId) {
