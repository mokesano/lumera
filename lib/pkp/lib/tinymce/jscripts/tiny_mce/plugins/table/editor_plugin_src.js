...
(function(tinymce) {
	...
	function hasContent(node) {
		var i, child, nodeName;

		if (node.nodeType === 3) // Text node
			return !!node.nodeValue.replace(/^\s+|\s+$/g, '').length;

		if (node.nodeType === 1) { // Element node
			nodeName = node.nodeName.toLowerCase();

			// Elements that should be treated as content
			if (/^(br|img|object|embed|input|textarea)$/.test(nodeName))
				return true;
		}
	...
	function isAtStart(rng, par) {
		...
		// Check for text characters or elements that should be treated as content
		return !hasContent(elm);
	};
