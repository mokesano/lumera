	...
	tinymce.create('tinymce.dom.DOMUtils', {
		...
		getAttribs : function(n) {
			...
			if (isIE) {
				...
				// Enumerate attributes from a shallow clone to avoid regex-based HTML parsing/sanitization issues
				var clone = n.cloneNode(false), attrs = clone.attributes, ai, attr;
				for (ai = 0; ai < attrs.length; ai++) {
					attr = attrs[ai];
					if (!attr)
						continue;

					// Keep behavior of only collecting specified attribute names
					if (attr.specified)
