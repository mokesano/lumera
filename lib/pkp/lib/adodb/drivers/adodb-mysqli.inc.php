...
class ADODB_mysqli extends ADOConnection {
    ...
    {
        // PHP 8.x: explicit constructor, all properties initialized via var declarations
    }
	...
	function ErrorMsg() 
	  {
        if (empty($this->_connectionID)) {
            ...
            // [WIZDAM FIX] PHP 8: mysqli_error() throws Error jika objek belum terkoneksi
            try {
...
class ADORecordSet_mysqli extends ADORecordSet{	
    ...
    function __construct($queryID, $mode = false) 
    {
      ...
      // Panggil parent __construct() untuk set _queryID dengan benar
      parent::__construct($queryID);	
    }
    
    // Pertahankan sebagai regular method untuk backward compat
    function ADORecordSet_mysqli($queryID, $mode = false) 
    {
        $this->__construct($queryID, $mode);
    }
	
    function _initrs()
    {
    global $ADODB_COUNTRECS;
        // PHP 8.x FIX: mysqli_num_rows/fields requires mysqli_result object.
        // For INSERT/UPDATE/DELETE, _queryID is true (not a result object).
...
*/
    ...
    {
        // PHP 8.x FIX: mysqli_field_seek/fetch_field requires mysqli_result object.
        if (!($this->_queryID instanceof mysqli_result)) {
    ...
    function _seek($row)
    {
      ...
      // PHP 8.x FIX: mysqli_data_seek() requires mysqli_result object.
    ...
    function MoveNext() 
    {
        ...
        // PHP 8.x FIX: mysqli_fetch_array() requires mysqli_result object.
        if (!($this->_queryID instanceof mysqli_result)) {
            $this->EOF = true;
            return false;
        }
    ...
    function _close() 
    {
        // PHP 8.x FIX: mysqli_free_result() requires mysqli_result object.
        if ($this->_queryID instanceof mysqli_result) {
